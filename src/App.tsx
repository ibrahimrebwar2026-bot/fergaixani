...
import {
  ...
  calculateNextStreak,
  calculateXpGain,
  Exercise as ProgressionExercise,
...
const stableShuffle = <T,>(arr: T[], seed: string): T[] => {
  const s = getHash(seed);
  const result = [...arr];
...
// Handle offline persistence with better error handling
...
const handleFirestoreError = (
  ...
  if (error?.code === "resource-exhausted") {
    const errorInfo = {
      error:
        "Quota exceeded. The daily limit for this database has been reached. It will reset in 24 hours.",
      operationType,
      path,
      isQuota: true,
    };
  ...
  if (!sessionStorage.getItem("ferga_quota_exceeded")) {
...
const cleanObject = (obj: any): any => {
  ...
  try {
    // A robust way to strip all non-JSON compatible objects (like DOM events, circular refs, Class instances)
    // while keeping data structures intact.
    return JSON.parse(
      JSON.stringify(obj, (k, v) =>
        v === undefined || typeof v === "function" ? undefined : v,
      ),
    );
  } catch (err) {
    console.warn("cleanObject JSON fallback failed", err);
    return obj;
  }
};
...
const testConnection = async () => {
  if (
    sessionStorage.getItem("ferga_conn_verified") ||
...
const SyncInput = ({
  ...
  return (
    <input
      ...
      onBlur={() => {
        if (localValue !== value) onSync(localValue);
      }}
      onKeyDown={(e) => {
...
const uploadImageToStorage = async (
  ...
  try {
    ...
    if (typeof base64OrFile === "string") {
      // Split base64 to remove the data:image/jpeg;base64, part if present
      const base64Data = base64OrFile.includes(",")
...
const dialectThemes: Record<TargetDialect, any> = {
  Sorani: {
    primary: "#b91c1c", // Deep rich red (Strength)
    primaryLight: "#ef4444",
    secondary: "#047857", // Emerald green (Tenderness)
    accent: "#f59e0b", // Golden yellow (Spirit)
    ring: "rgba(185, 28, 28, 0.3)",
    gradient: "from-[#0f0404] via-[#1a0a0a] to-[#041a10] bg-[size:100%_100%]",
    darkBg: "#050101",
    darkText: "#fdfbfa",
    lightBg: "#fef2f2",
    lightText: "#450a0a",
  ...
  Kurmanji: {
    primary: "#047857", // Emerald green (Tenderness)
    primaryLight: "#10b981",
    secondary: "#b91c1c", // Deep red (Strength)
    accent: "#fbbf24", // Yellow (Spirit)
    ring: "rgba(4, 120, 87, 0.3)",
    gradient: "from-[#02100a] via-[#051f15] to-[#140606] bg-[size:100%_100%]",
    darkBg: "#010503",
    darkText: "#fdfbfa",
    lightBg: "#f0fdf4",
    lightText: "#064e3b",
  ...
  Hawrami: {
    primary: "#7e22ce", // Deep Royal Purple
    primaryLight: "#a855f7",
    secondary: "#b91c1c", // Deep red
    accent: "#fbbf24", // Gold
    ring: "rgba(126, 34, 206, 0.3)",
    gradient: "from-[#0b0312] via-[#160a24] to-[#120404] bg-[size:100%_100%]",
    darkBg: "#030105",
    darkText: "#fdfbfa",
    lightBg: "#fdf4ff",
    lightText: "#3b0764",
  ...
  Zazaki: {
    primary: "#0369a1", // Deep Azure (Spirit/Air)
    primaryLight: "#0ea5e9",
    secondary: "#f59e0b", // Gold (Spirit)
    accent: "#047857", // Green
    ring: "rgba(3, 105, 161, 0.3)",
    gradient: "from-[#020e14] via-[#041d2e] to-[#0a1a0f] bg-[size:100%_100%]",
    darkBg: "#010406",
    darkText: "#fdfbfa",
    lightBg: "#f0f9ff",
    lightText: "#082f49",
  ...
  Luri: {
    primary: "#be123c", // Deep Ruby
    primaryLight: "#f43f5e",
    secondary: "#f59e0b", // Amber
    accent: "#047857", // Emerald
    ring: "rgba(190, 18, 60, 0.3)",
    gradient: "from-[#120307] via-[#210611] to-[#141002] bg-[size:100%_100%]",
    darkBg: "#070102",
    darkText: "#fdfbfa",
    lightBg: "#fff1f2",
    lightText: "#4c0519",
...
const KurdistanMap = ({ className = "" }: { className?: string }) => (
  <div className={`pointer-events-none select-none ${className}`}>
    <svg
      viewBox="0 0 100 100"
      className="w-full h-full fill-white/[0.1] stroke-white/20"
      strokeWidth="0.5"
    >
...
const ThemeInjected = ({
  ...
  const pageBg = isDark ? theme.darkBg : theme.lightBg;
  const pageText = isDark ? theme.darkText : theme.lightText;
      ...
      dangerouslySetInnerHTML={{
        __html: `
            ...
            body { 
                ...
                font-family: 'UniQAIDAR_Afarin_002', 'Inter', sans-serif !important;
            ...
            .theme-text-soft { color: ${isDark ? "rgba(255, 255, 255, 0.7)" : "rgba(0, 0, 0, 0.7)"}; }
            .theme-text-muted { color: ${isDark ? "rgba(255, 255, 255, 0.5)" : "rgba(0, 0, 0, 0.5)"}; }
            .theme-bg-soft { background-color: ${isDark ? "rgba(255, 255, 255, 0.05)" : "rgba(0, 0, 0, 0.03)"}; }
            .theme-border-soft { border-color: ${isDark ? "rgba(255, 255, 255, 0.1)" : "rgba(0, 0, 0, 0.1)"}; }
            .theme-border-soft-50 { border-color: ${isDark ? "rgba(255, 255, 255, 0.05)" : "rgba(0, 0, 0, 0.05)"}; }
            .theme-bg-secondary { background-color: var(--secondary); }
            .theme-shadow-primary { box-shadow: 0 0 30px -5px var(--primary); }
            ...
            .theme-card:hover, .theme-bg-card:hover {
                ...
                box-shadow: ${isDark ? "0 50px 100px -20px rgba(0,0,0,0.9), inset 0 1px 1px rgba(255,255,255,0.1)" : "0 30px 60px -10px rgba(0,0,0,0.12), inset 0 1px 1px rgba(255,255,255,1)"};
            }
            ...
            .interactive-card:hover {
                transform: translateY(-2px);
                background-color: ${isDark ? "rgba(255, 255, 255, 0.05)" : "rgba(0, 0, 0, 0.01)"};
                box-shadow: ${isDark ? "0 20px 40px -15px rgba(0,0,0,0.6)" : "0 15px 35px -5px rgba(0,0,0,0.1)"};
            }
        `,
      }}
    />
...
interface Exercise {
  ...
  type:
    ...
    | "construct"
    | "intro";
  question: string | { Sorani: string; Kurmanji: string };
  imageUrl?: string;
  options?: string[];
...
const ACHIEVEMENTS: Achievement[] = [
  // --- LESSON CHALLENGES ---
  {
    ...
    description: {
      Sorani: "یەکەم وانەت بە سەرکەوتوویی تەواو کرد",
      Kurmanji: "Te dersa xwe ya yekem qedand",
    },
    icon: "target",
  ...
  },
  {
    ...
    description: {
      Sorani: "١٠ وانەت تەواو کردووە",
      Kurmanji: "Te 10 ders qedandin",
    },
    icon: "book",
  ...
  },
  {
    ...
    description: {
      Sorani: "٥٠ وانەت بڕیوە، شکۆمەندیت!",
      Kurmanji: "Te 50 ders qedandin, pîroz be!",
    },
    icon: "award",
  ...
  // --- XP MILESTONES ---
  {
    ...
    description: {
      Sorani: "گەیشتیتە ١٠٠ خاڵی توانست",
      Kurmanji: "Te 100 XP bi dest xist",
    },
    icon: "zap",
  ...
  },
  {
    ...
    description: {
      Sorani: "١٠٠٠ خاڵی توانستت کۆکردەوە",
      Kurmanji: "Te 1000 XP kom kir",
    },
    icon: "star",
  ...
  },
  {
    ...
    description: {
      Sorani: "٥٠٠٠ خاڵ! ئەتۆ ئەفسانەییت",
      Kurmanji: "5000 XP! Tu efsane yî",
    },
    icon: "trophy",
  ...
  // --- STREAK HEROES ---
  {
    ...
    description: {
      Sorani: "٣ ڕۆژ بەردەوامی لە فێربوون",
      Kurmanji: "3 roj berdewamiya fêrbûnê",
    },
    icon: "flame",
  ...
  },
  {
    ...
    description: {
      Sorani: "٧ ڕۆژ بێ وەستان فێربووی",
      Kurmanji: "7 roj bê rawestiyan",
    },
    icon: "calendar",
  ...
  },
  {
    ...
    description: {
      Sorani: "مانگێکی تەواو بەردەوام بوویت",
      Kurmanji: "Mehekê te berdewam kir",
    },
    icon: "crown",
  ...
  // --- DIALECT SPECIFIC ---
  {
    ...
    description: {
      Sorani: "٥ وانەی سۆرانیت تەواو کرد",
      Kurmanji: "Te 5 dersên Soranî qedandin",
    },
    icon: "languages",
  ...
  },
  {
    ...
    description: {
      Sorani: "٥ وانەی کورمانجیت تەواو کرد",
      Kurmanji: "Te 5 dersên Kurmancî qedandin",
    },
    icon: "languages",
  ...
  },
  {
    ...
    description: {
      Sorani: "٢ وانەی هەورامیت تەواو کرد",
      Kurmanji: "Te 2 dersên Hewramî qedandin",
    },
    icon: "languages",
  ...
  },
  {
    ...
    description: {
      Sorani: "٢ وانەی زازاکیت تەواو کرد",
      Kurmanji: "Te 2 dersên Zazakî qedandin",
    },
    icon: "languages",
  ...
  },
  {
    ...
    description: {
      Sorani: "٢ وانەی لوڕیت تەواو کرد",
      Kurmanji: "Te 2 dersên Lurî qedandin",
    },
    icon: "languages",
  ...
  // --- SPECIAL MIX ---
  {
    ...
    description: {
      Sorani: "لە هەموو شێوەزارەکان وانەت خوێندوە",
      Kurmanji: "Te di her devokekê de ders xwendine",
    },
    icon: "globe",
...
const dialectAlphabets: Record<
  string,
  { chars: { char: string; desc: { Sorani: string; Kurmanji: string } }[] }
> = {
  sorani: {
    chars: [
      {
        char: "ڕ",
        desc: {
          Sorani: "ڕی قورس: بە دەنگێکی بەهێزتر دەردەبڕدرێت وەک لە 'ر'ی ئاسایی.",
          Kurmanji: "ڕ (R): Bi dengekî xurttir tê derxistin ji 'r'ya asayî.",
        },
      },
      {
        char: "ڵ",
        desc: {
          Sorani: "ڵی قورس: لە دواوەی زمان دەردەبڕدرێت وەک لە 'وڵات'.",
          Kurmanji: "ڵ (L): Ji paşiya zimên tê derxistin wek di 'Welat' de.",
        },
      },
      {
        char: "ڤ",
        desc: {
          Sorani:
            "پیتی ڤ: دەنگێکی هاوشێوەی 'V'ی ئینگلیزی هەیە. نموونە: 'تاڤگە'.",
          Kurmanji: "Tîpa ڤ (V): Dengê 'V' yê îngilîzî dide. Mînak: 'Tavge'.",
        },
      },
      {
        char: "ۆ",
        desc: {
          Sorani: "پیتی ۆ: دەنگی 'O'یەکی درێژ دەدات.",
          Kurmanji: "Tîpa ۆ (O): Dengê 'O'yekî dirêj dide.",
        },
      },
      {
        char: "ێ",
        desc: {
          Sorani: "پیتی ێ: دەنگی 'E'یەکی درێژ دەدات (وەک 'Hey').",
          Kurmanji: "Tîpa ێ (E): Dengê 'E'yekî dirêj dide (wek 'Hey').",
        },
      },
    ],
  },
    ...
    chars: [
      {
        char: "Ç",
        desc: {
          Sorani: "پیتی Ç: دەنگی 'چ' دەدات. نموونە: 'Çav' (چاو).",
          Kurmanji: "Tîpa Ç: Dengê 'ç' dide. Mînak: 'Çav'.",
        },
      },
      {
        char: "Ş",
        desc: {
          Sorani: "پیتی Ş: دەنگی 'ش' دەدات. نموونە: 'Şew' (شەو).",
          Kurmanji: "Tîpa Ş: Dengê 'ş' dide. Mînak: 'Şew'.",
        },
      },
      {
        char: "Ê",
        desc: {
          Sorani: "پیتی Ê: دەنگی 'ێ'ی سۆرانی دەدات. نموونە: 'Dê'.",
          Kurmanji: "Tîpa Ê: Dengê 'ê' ya Soranî dide. Mînak: 'Dê'.",
        },
      },
    ],
  },
    ...
    chars: [
      {
        char: "ۆ",
        desc: {
          Sorani: "ۆ (O): لە هەورامیدا زۆر ناو دەستنیشان دەکات لە کۆتایی وشە.",
          Kurmanji: "ۆ (O): Di Hewramî de ji bo nîşandana navan tê bikaranîn.",
        },
      },
      {
        char: "ڎ",
        desc: {
          Sorani: "ڎ: ئەم پیتە تایبەتە بە هەورامی، دەنگێکی 'د'ی قورسی هەیە.",
          Kurmanji: "ڎ: Ev tîp taybet e bi Hewramî, dengê 'd'yek xurt e.",
        },
      },
      {
        char: "ێ",
        desc: {
          Sorani: "ێ (E): دەنگی 'ێ'ی درێژ لە کۆتایی ناوە مێینەکاندا زۆرە.",
          Kurmanji: "ێ (E): Dengê 'ê' yê dirêj di navên mê de pir heye.",
        },
      },
    ],
  },
    ...
    chars: [
      {
        char: "Ğ",
        desc: {
          Sorani: "Ğ: پیتی 'غ'ی سۆرانییە کە بە شێوەیەکی نەرم دەردەبڕدرێت.",
          Kurmanji: "Ğ: Tîpa 'ğ' ye ku bi dengekî nerm tê xwendin.",
        },
      },
      {
        char: "Ê",
        desc: {
          Sorani: "Ê: پیتی 'ێ'ی سۆرانی دەدات. نموونە: 'Zerrê'.",
          Kurmanji: "Ê: Dengê 'ê' ya Soranî dide. Mînak: 'Zerrê'.",
        },
      },
      {
        char: "Û",
        desc: {
          Sorani: "Û: دەنگی 'وو'ی سۆرانی دەدات.",
          Kurmanji: "Û: Dengê 'û' yê dirêj (wek 'û' ya Soranî) dide.",
        },
      },
    ],
  },
    ...
    chars: [
      {
        char: "ڤ",
        desc: {
          Sorani: "ڤ: ئەم دەنگە لە زۆربەی زاراوەکانی لوڕیدا بە باوی بەکاردێت.",
          Kurmanji: "ڤ (V): Ev deng di piraniya devokên Lurî de bi kar tê.",
        },
      },
      {
        char: "ۆ",
        desc: {
          Sorani: "ۆ: دەنگێکی کورت و خێرایە لە هەندێک وشەی لوڕیدا.",
          Kurmanji: "ۆ: Dengê 'O'yê ye ku di Lurî de xurt e.",
        },
      },
    ],
  },
...
const uiLabels = {
  Sorani: {
    ...
    missionDesc:
      "فێرگە هەوڵێکە بۆ پاراستن و برەودان بە شێوەزارە جیاوازەکانی زمانی کوردی و نزیککردنەوەی نەوەی نوێ لە ڕەچەڵەک و مێژووی خۆی.",
    teamDesc:
      "ئێمە گروپێک لە دڵسۆزانی زمان و تەکنەلۆژیاین کە دەمانەوێت بە ئامرازە نوێیەکان خزمەت بە زمانی دایک بکەین.",
    cultureDesc:
      "زمانی کوردی یەکێکە لە دەوڵەمەندترین زمانەکان و فێربوونی شێوەزارەکانی نیشانەی زیندوویی و یەکڕیزی کولتووری ئێمەیە.",
  ...
  Kurmanji: {
    ...
    confirmDelete:
      "Ma tu ji jêbirina hesaba xwe bawer î? Ev kiryar nayê vegerandin.",
    open: "Veke",
    ...
    missionDesc:
      "Fêrge hewildanek e ji bo parastin û pêşvebirina devokên cihêreng ên zimanê kurdî û nêzîkkirina nifşê nû ji koka xwe û dîroka xwe.",
    teamDesc:
      "Em komek dilxwazên ziman û teknolojiyê ne ku dixwazin bi amûrên nûjen xizmeta zimanê dayikê bikin.",
    cultureDesc:
      "Zimanê kurdî yek ji zimanên herî dewlemend e û fêrbûna devokên wî nîşana zindîbûn û yekitiya çanda me ye.",
...
const getUnitAdvice = (unitId: number) => {
  ...
  > = {
    1: {
      sorani:
        "بنەماکان بناغەی زمانن. سڵاوکردن لە کولتووری کوردیدا نیشانەی ڕێز و کردنەوەی دەرگای دڵە. هەوڵ بدە لە دەربڕینی وشەکاندا ئاوازی زمانەکە بپارێزیت. فێربوونی پیتەکان و دەنگەکان یەکەم هەنگاوە بەرەو جیهانێکی نوێ.",
      kurmanji:
        "Bingeh bingeha ziman in. Silavdayîn di çanda Kurdî de nîşana rêzgirtinê û vekirina deriyên dil e. Hewl bide di axaftina peyvan de muzîka ziman biparêzî. Hînbûna tîp û bengan gava yekem e ber bi cîhanek nû.",
    ...
    2: {
      sorani:
        "خێزان و ماڵ و خواردن، سێکوچکەی ژیانی کوردەوارین. ناسینی ناوەکانی خزمایەتی و پێکهاتەی خواردنەکان، تۆ دەباتە ناو قووڵایی میوانداری و خەسڵەتی خێزانی کورد. هەرگیز لە بیرت نەچێت کە سفرەی کورد هەمیشە بۆ میوان ئاوەڵایە.",
      kurmanji:
        "Malbat, mal û xwarin, bingeha jiyana Kurdewarî ne. Naskirina navên xizmtiyê û pêkhateyên xwarinan, te dimeşîne kûrahiya mêvandarî û taybetmendiya malbata Kurd.",
    ...
    3: {
      sorani:
        "سروشت و ئاژەڵان ناسنامەی چیاکانی کوردستانن. کورد و شاخ هەمیشە هاوڕێی یەک بوون. فێربوونی ئەم وشانە، پەیوەندیت لەگەڵ ژینگەی ڕەسەنی کوردستان بەهێز دەکات. پاراستنی ژینگە پاراستنی ژیانە.",
      kurmanji:
        "Xweza û heywan nasnameya çiyayên Kurdistanê ne. Kurd û çiya her tim hevalên hev bûne. Hînbûna van peyvan, têکiliya te ya bi jîngeha resen a Kurdistanê re xurtir dike.",
      ...
      meaningKurmanji:
        "Nîşana wê yekê ye ku çiya her tim bûne star û parêzerê Kurdan.",
    },
    4: {
      sorani:
        "کردارەکان هێز دەدەنە دەربڕینەکانت و جەستەش ئامرازی جێبەجێکردنیانە. فێربوونی کردارە بنەڕەتییەکان و تێگەیشتن لە جەستەی مرۆڤ، بنەمایەکی گرنگە بۆ گفتوگۆیەکی زیندوو. زمان بە کردارەوە دەدرەوشێتەوە.",
      kurmanji:
        "Lêker hêzê didin derbirînên te û gewde jî amraza pêkanîna wan e. Hînbûna lêkerên bingehîn û fêmkirina gewdeyê mirovan, bingehê axaftineke zindî ye.",
      ...
      meaningKurmanji:
        "Dema kesek armancên pir mezin datîne lê baweriya wî bi xwe nîn e.",
    },
    5: {
      sorani:
        "ڕەنگەکان و کات و وەسف، جوانی دەدەنە زمان. لە کەلتووری کوردیدا ڕەنگەکان مانا و هێمای جیاوازیان هەیە و کاتیش بەپێی وەرزەکان و سروشت گوزارشتی لێ دەکرێت. هەر ساتێک دەرفەتێکە بۆ فێربوون.",
      kurmanji:
        "Reng, dem û wesf, bedewiyê didin ziman. Di çanda Kurdî de reng xwedî wateyên cuda ne û dem jî li gorî werz û xweزayê tê derbirîn.",
    ...
    6: {
      sorani:
        "زانست و خوێندن کلیلی داهاتوویەکی ڕووناکە. بەکارهێنانی زاراوە ئەکادیمی و زانستییەکان بە زمانی دایک، یارمەتیدەرە بۆ گەشەسەندنی فیکری و پاراستنی زمان لە فەوتان. هەوڵ بدە هەمیشە بەدوای زانیندا بگەڕێیت.",
      kurmanji:
        "Zanist û xwendin kilîta pašerojeke ronî ye. Bikaranîna terîmên akademîк û زانیstî bi zimanê dayikê, alîkar e ji bo gešepêdana fikrî û parastina ziman.",
    ...
    7: {
      sorani:
        "ژمارەکان زمانی ژاکاو و بازرگانی و هەموو بوارەکانی ژیانن. بێ زانینی ژمارەکان، پەیوەندییە دارایی و کاتییەکانی مرۆڤ پەکدەست دەبن. ژماردن و حسابکردن بەشێکی دانەبڕاوی ڕۆژانەیە.",
      kurmanji:
        "Hejmar zimanê بازرگانی û hemû qadên jiyanê ne. Bêyî naskirina hejmaran, têکiliyên darayî û yên demê yên mirovan kêm dimînin.",
      idiomSorani: "حساب حسابە، کاکا برایە",
      meaningSorani:
        "لە کار و حسابدا دەبێت ورد بیت، تەنانەت لەگەڵ نزیکترین کەسیش.",
      idiomKurmanji: "Hesab hesab e, bira bira ye",
      meaningKurmanji:
        "Divê di کار û حساب de meriv cidî be, her چiqas dostanî hebe jî.",
    },
    8: {
      sorani:
        "مێژوو و کەلتوور ڕەگی قووڵی نەتەوەن. ناسینی شوێنەوار و دەستنووس و ڕووداوە مێژووییەکان، زانیاریت لەسەر شووناس و ڕەسەنایەتی خۆت زیاد دەکات. نەتەوەی بێ مێژوو وەک داری بێ ڕەگ وایە.",
      kurmanji:
        "Dîrok û čand koka kûr a neteweyan in. Naskirina šûnewar, destnivîs û rûdanên dîrokî, zaniariyan li ser nasname û resenatiya te zêde dike.",
      idiomSorani: "داری بێ ڕەگ وشک دەبێت",
      meaningSorani:
        "کەسێک کە ڕابردوو و کەلتووری خۆی لەبیر بکات، داهاتووی نابێت.",
      idiomKurmanji: "Darê bê kok hišk dibe",
      meaningKurmanji: "Mirovê ku čand û koka xwe nas neke, zû winda dibe.",
    },
    9: {
      sorani:
        "میوەکان سەرچاوەی وزە و بەشی دانەبڕاوی کوردستانن. کوردستان بە هەنار و هەنجیر و سێوی جیاواز بەناوبانگە کە هەریەکەیان چیرۆکێکی کشتوکاڵی و خۆشییان هەیە. لێرەدا فێری دەبیت چۆن وەسفی تامی خۆشی میوەکان بکەیت.",
      kurmanji:
        "Fêkî čavkaniya enerjiyê û bešeke bingehîn a Kurdistanê ne. Kurdistan bi henar, hejîr û sêvên xwe yên cuda navdar e.",
      idiomSorani: "میوەی گەییو بۆ خۆی دەکەوێت",
      meaningSorani:
        "کارێک کاتەکەی هات، بۆ خۆی جێبەجێ دەبێت و پێویستی بە پەلە نییە.",
      idiomKurmanji: "Fêkiyê gihîštî xwe bi xwe dikeve",
      meaningKurmanji: "Dema wextê tištekî hat, ew bi serê خۆ پێک tê.",
    },
    10: {
      sorani:
        "سەوزەکان بنەمای سفرەی تەندروستی کوردین. زۆرێک لە سەوزە کێوییەکانی ناو چیاکانی کوردستان، هەم خواردن و هەم دەرمانی سروشتین. فێربونی ناوی ئەمانە یارمەتیت دەدات بۆ تێگەیشتن لە کەلتووری خواردەمەنی.",
      kurmanji:
        "Sebze bingeha sifreya tendurist a Kurdî ne. Gelek ji sebzeyên kovî yên li čiyayên Kurdistanê, hem xwarin û hem jî dermanên xweزayî ne.",
      idiomSorani: "سەوزە بە ئاو شین دەبێت",
      meaningSorani:
        "هەر شتێک پێویستی بە سەرچاوە و یارمەتی هەیە بۆ ئەوەی گەشە بکات.",
      idiomKurmanji: "Sebze bi avê شîn dibe",
      meaningKurmanji: "Her tišt bi ked û alîkariyê geش dibe, bê čavkanî nabe.",
    },
    11: {
      sorani:
        "گەشت و سەفەر ئاسۆی بیرکردنەوە فراوان دەکات. فێربوونی ناوی شوێنەکان و وشەی گەشتوگوزار، یارمەتیدەرە بۆ ئەوەی لە هەر گەشتێکدا باشتر پەیوەندی ببەستیت. گەورەیی جیهان لە گەشتدا دەبینرێت.",
      kurmanji:
        "Gešt û sefar asoya ramanê firehtir dike. Hînbûna navên cîhan û peyvên geštiyariyê, alîکار e daku tu di ہر geštekê de têkiliyeke baštir deynî.",
      idiomSorani: "گەشت زانیارییە",
      meaningSorani:
        "بینینی وڵاتان و شوێنە جیاوازەکان وەک خوێندنەوەی کتێبێک وایە.",
      idiomKurmanji: "Ger û gešt zanyarî ye",
      meaningKurmanji: "Mirov ہر ku bigere, ew qas zilm û zanێنê زêde dike.",
    },
    12: {
      sorani:
        "کار و بازرگانی کۆڵەکەی ئابوورین. زانینی ئەم زاراوانە لە جیهانی کاردا، دەرفەتی زیاترت بۆ دەڕەخسێنێت و متمانە بە زمانەکەت زیاتر دەکات. قسە و مامەڵە لە بازاڕدا هونەرە.",
      kurmanji:
        "Kar û بازرگانی stûnên aboriyê ne. Zanîna van terîمان di cîhana کار de, derfetên زêdetir ji bo te peyda dike û baweriya te bi zمانê te زêdetir dike.",
      idiomSorani: "دەست و برد لە کاردا فەرزە",
      meaningSorani:
        "دەبێت لە کاردا خێرا و لێهاتوو بیت بۆ ئەوەی سەرکەوتوو بیت.",
      idiomKurmanji: "Dest û ling di kar de pir girîng in",
      meaningKurmanji:
        "Divê mirov di kar de čust û čalak be da ku serketî bibe.",
    },
    13: {
      sorani:
        "هونەر و ئەدەب زمانی ڕۆحی کوردین. شیعر و موزیک هەمیشە هاوڕێی مێژووی پڕ هەور و نشێوی ئێمە بوون و پارێزەری شووناسمان بوون. لێرەدا دەچیتە ناو دڵی چاند و فەرهەنگ.",
      kurmanji:
        "Huner û edeb zimanê ruhê Kurdî ne. Helbest û موزîک ہر tim hevalên dîroka me ya dagirtî bi bilindî û nizmiyan bûne.",
    ...
    14: {
      sorani:
        "تەندروستی و هەستەکان دووانەیەکی گرنگی مرۆڤن. گوزارشتکردن لە ئازار و خۆشی، و تێگەیشتن لە باری تەندروستی، یەکەم هەنگاوە بۆ ژیانێکی باشتر. مێشکی ساغ لە لەشی ساغدایە.",
      kurmanji:
        "Tenduristî û hest duwemeke girîng a mirov in. Derbirîna êš û šahiyan, û fêmkirina rewšta tenduristiyê, gava yekem e bo jiyaneke baštir.",
      ...
      meaningKurmanji:
        "Mirovê saxlem mîna padîšah e, tenduristî dewlemendiya هری mezin e.",
    },
    15: {
      sorani:
        "ئاستی پێشکەوتوو واتە قووڵبوونەوە لە فیکر و فەلسەفەی زمان. لێرەدا تۆ واتای وشەکان لە چوارچێوەیەکی فراوانتر و ژیرانەتردا بەکاردەهێنیت. ئێستا تۆ دەتوانی ببی بە مامۆستایەک بۆ ئەوانی تریش.",
      kurmanji:
        "Asta پێšketî tê wateya kûrbûna di fikir û felsefeya ziman de. Li vir tu wateya peyvan di čarčoveyeke berfirehtir û biaqiltir de bi kar tînî.",
...
const getUnitData = (
  ...
  const dialectKey = target.toLowerCase() as
    | "sorani"
    | "kurmanji"
    | "hawrami"
    | "zazaki"
    | "luri";
  const interfaceKey = interfaceLang.toLowerCase() as
    | "sorani"
    | "kurmanji"
    | "hawrami"
    | "zazaki"
  ...
  const seededShuffle = <T,>(arr: T[], seed: number | string): T[] => {
  ...
  const getPronoun = (p: string) =>
    (kurdishDictionary.pronouns as any)[p]?.[dialectKey] || p;

  const baseDict = kurdishDictionary.dictionary;
  const extraDict = appConfig?.customDictionary || [];

  // Merge core and custom - custom overrides core by ID or Sorani word
  ...
  const unitDefinitions = [
    // Level 1: Beginner (Foundations)
    {
      ...
      name: isSorani ? "بنەماکانی زمان" : "Bingehên Ziman",
    ...
    },
    {
      id: 2,
      categories: ["Numbers"],
      name: isSorani ? "ژمارەکان" : "Hejmar",
      theme: "from-fuchsia-600 to-pink-700",
      motif: "star",
      level: 1,
    },
    {
    ...
    },
    {
      ...
      name: isSorani ? "ماڵ و خواردن" : "Mal u Xwarin",
      theme: "from-amber-600 to-orange-700",
      motif: "home",
      level: 1,
    },

    // Level 2: Elementary (Daily Life)
    {
      id: 5,
      categories: ["Nature", "Animals"],
      name: isSorani ? "سروشت و ئاژەڵ" : "Xweza u Heywan",
    ...
    },
    {
      ...
      name: isSorani ? "جەستە و تەندروستی" : "Gewde u Tenduristî",
      theme: "from-red-600 to-rose-800",
      motif: "heart",
      level: 2,
    },
    {
      ...
      name: isSorani ? "ڕەنگ و وەسف" : "Reng u Wesf",
    ...
    },
    {
      id: 8,
      categories: ["Time"],
      name: isSorani ? "کات و ژیان" : "Dem u Jiyan",
    ...
    // Level 3: Intermediate (Communication)
    {
      id: 9,
      categories: ["Verbs"],
      name: isSorani ? "کردار و جوڵە" : "Lêker u Tevger",
      theme: "from-purple-600 to-indigo-800",
      motif: "zap",
      level: 3,
    },
    {
      ...
      name: isSorani ? "زانست و فێربوون" : "Zanist u Hînbûn",
    ...
    },
    {
      ...
      name: isSorani ? "گەشت و شوێن" : "Geşt u Cîh",
      theme: "from-cyan-600 to-blue-800",
      motif: "map",
      level: 3,
    },
    {
      ...
      name: isSorani ? "ئیش و بازرگانی" : "Kar u Bazirganî",
      theme: "from-slate-700 to-slate-900",
      motif: "briefcase",
      level: 3,
    },

    // Level 4: Advanced (Culture & Abstraction)
    {
      id: 13,
      categories: ["History", "Culture"],
      name: isSorani ? "مێژوو و کەلتوور" : "Dîrok u Çand",
      theme: "from-amber-700 to-orange-800",
      motif: "scroll",
      level: 4,
    },
    {
      id: 14,
      categories: ["Arts", "Emotions"],
      name: isSorani ? "هونەر و هەست" : "Huner u Hest",
      theme: "from-rose-700 to-purple-900",
      motif: "pen",
      level: 4,
    },
    {
      id: 15,
      categories: ["Advanced"],
      name: isSorani ? "وشەی پێشکەوتوو" : "Peyvên Pêşketî",
      theme: "from-zinc-700 to-zinc-900",
      motif: "crown",
      level: 4,
    },
  ];
  ...
  const getDialectName = (d: string) => {
    switch (d) {
      case "Sorani":
        return isSorani ? "سۆرانی" : "Soranî";
      case "Kurmanji":
        return isSorani ? "کورمانجی" : "Kurmancî";
      case "Hawrami":
        return isSorani ? "هەورامی" : "Hewramî";
      case "Zazaki":
        return isSorani ? "زازاکی" : "Zazakî";
      case "Luri":
        return isSorani ? "لوڕی" : "Lurî";
      default:
        return d;
    }
  };
  ...
  const getSOVLesson = (
    ...
    const contentMap: Record<string, any> = {
      sorani: {
        intro: {
          Sorani:
            "ڕستەسازی سۆرانی بەزۆری (بکەر - بەرکار - کردار)ـە. وەک: من نان دەخۆم.",
          Kurmanji:
            "Rêzimana Soranî (Biker - Berkar - Lêker) e. Wek: Min nan dexom.",
        },
        idioms: [
          {
            phrase: "هەموو شتێک بە خوێ، خوێش بە ماناوە.",
            meaning: {
              Sorani: "هەموو شتێک پێویستی بە هاوسەنگی و ژیری هەیە.",
              Kurmanji:
                "Her tişt bi xwê, xwê jî bi manayê. Yanî her tişt bi hevsengî û zanyariyê xweş e.",
            },
            context: {
              Sorani:
                "ئەم پەندە کاتێک بەکاردێت کە باس لە گرنگی عەقڵ و دانایی بکرێت لە کارەکاندا.",
              Kurmanji:
                "Ev pend dema ku qala girîngiya aqil û zanabûnê di karan de tê kirin, tê bikaranîn.",
            },
          },
          {
            phrase: "خەریکی چییت؟",
            meaning: {
              Sorani: "بە واتای ئەوە دێت کە ئێستا چ دەکەیت؟",
              Kurmanji: "Tê wateya 'Tu bi çi re mijûl î?'",
            },
            context: {
              Sorani:
                "شێوازێکی باوی پرسینی بارودۆخی کەسێکە بە شێوەیەکی نافەرمی.",
              Kurmanji:
                "Şêweyekî asayî yê pirsîna rewşa kesekî ye bi awayekî nefermî.",
            },
          },
        ],
        exercises: [
          {
            q: "نان دەخۆم من",
            a: "من نان دەخۆم",
            options: ["من", "نان", "دەخۆم"],
          },
          {
            q: "ئەو دەڕوات بۆ ماڵەوە",
            a: "ئەو بۆ ماڵەوە دەڕوات",
            options: ["ئەو", "بۆ", "ماڵەوە", "دەڕوات"],
          },
      ...
      kurmanji: {
        intro: {
          Sorani:
            "ڕستەسازی کرمانجی (Biker - Berkar - Lêker)ـە. وەک: Ez nanî dixwim.",
          Kurmanji:
            "Rêziknameya hevokan di Kurmancî de (Biker - Berkar - Lêker) e. Wek: Ez nanî dixwim.",
        },
        idioms: [
          {
            ...
            meaning: {
              Sorani:
                "شێر هەر شێرە، چ مێ بێت یان نێر. ئاماژەیە بۆ یەکسانی و ئازایەتی.",
              Kurmanji:
                "Şêr şêr e, heta jin be an mêr be. Nîşana wekhevî û wêrekiyê ye.",
            },
            context: {
              Sorani:
                "ئەم پەندە بۆ نیشاندانی ئەوە بەکاردێت کە لێهاتوویی پەیوەندی بە ڕەگەزەوە نییە.",
              Kurmanji:
                "Ev pend ji bo nîşandana wê yekê tê bikaranîn ku huner û jêhatîbûn ne girêdayî zayendê ye.",
            },
          },
          {
            phrase: "Çavê min li rê ye.",
            meaning: {
              Sorani: "چاوەڕێم یان بە تەمام.",
              Kurmanji: "Ez li benda te me an jî ez çavdar im.",
            },
            context: {
              Sorani:
                "کاتێک بەکاردێت کە بە پەرۆشەوە چاوەڕێی هاتنی کەسێک یان ڕووداوێک بیت.",
              Kurmanji:
                "Dema ku tu bi kelecan li benda hatina kesekî an bûyerekî bî, tê bikaranîn.",
            },
          },
        ],
        exercises: [
          {
            q: "dixwim nanî Ez",
            a: "Ez nanî dixwim",
            options: ["Ez", "nanî", "dixwim"],
          },
          {
            q: "diçî malê Tu",
            a: "Tu diçî malê",
            options: ["Tu", "diçî", "malê"],
          },
      ...
      hawrami: {
        intro: {
          Sorani:
            "ڕستەسازی هەورامی (بکەر - بەرکار - کردار)ـە. وەک: من نان مۆرو.",
          Kurmanji:
            "Rêzimana Hewramî (Biker - Berkar - Lêker) e. Wek: Min nan mewanu.",
        },
        idioms: [
          {
            ...
            meaning: {
              Sorani:
                "ڕاستەوخۆ دەڵێت ڕێگای باڵندەکان بە کایە داپۆشراوە. مەبەست لێی ڕێگایەکی نادیار یان ناسکە.",
              Kurmanji:
                "Riya çivîkan bi kayê hatiye girtin. Yanî riyeke nediyar an jî nazik e.",
            },
            context: {
              Sorani:
                "کاتێک بەکاردێت کە باس لە کارێکی قورس یان ڕێگایەکی نەزانراو بکرێت.",
              Kurmanji:
                "Dema ku qala karekî dijwar an riyeke nenas tê kirin, tê bikaranîn.",
            },
          },
        ],
        exercises: [
      ...
      luri: {
        intro: {
          Sorani:
            "ڕستەسازی لوڕی (بکەر - بەرکار - کردار)ـە. وەک: مه نون می هورم.",
          Kurmanji:
            "Rêzimana Lurî (Biker - Berkar - Lêker) e. Wek: Me nûn mi hwerum.",
        },
        idioms: [
          {
            phrase: "تیەم ڕێ.",
            meaning: {
              Sorani: "چاو لە ڕێگام، واتە چاوەڕێم.",
              Kurmanji: "Çav li rê me, yanî ez li bendê me.",
            },
            context: {
              Sorani: "بۆ دەربڕینی هەستی پەرۆشی بۆ بینینی کەسێک بەکاردێت.",
              Kurmanji:
                "Ji bo nîşandana hestê kelecaniyê ji bo dîtina kesekî tê bikaranîn.",
            },
          },
        ],
        exercises: [
      ...
      zazaki: {
        intro: {
          Sorani:
            "ڕستەسازی زازاکی (Biker - Berkar - Lêker)ـە. وەک: Ez nan wenan.",
          Kurmanji:
            "Rêzimana Zazakî (Biker - Berkar - Lêker) e. Wek: Ez nan wenan.",
        },
        idioms: [
          {
            phrase: "Mase derya de nêroşîyeno.",
            meaning: {
              Sorani: "ماسی لە ناو دەریا نافرۆشرێت.",
              Kurmanji: "Masî di deryayê de nayê firotin.",
            },
            context: {
              Sorani:
                "کاتێک بەکاردێت کە کەسێک بیەوێت شتێک بفرۆشێت کە هێشتا دەستی نەکەوتووە.",
              Kurmanji:
                "Gava ku kesek bixwaze tiştekî bifiroşe ku hîna bi dest nexistiye, tê gotin.",
            },
          },
        ],
        exercises: [
    ...
    if (alphabetInfo) {
      alphabetInfo.chars.forEach((c, idx) => {
        exercises.push({
          ...
          type: "intro",
          question: {
            Sorani: `پیتی تایبەت: ${c.char}`,
            Kurmanji: `Tîpa Taybet: ${c.char}`,
          },
          answer: "",
    ...
    if (khaniIdioms.length > 0) {
      const idiom = khaniIdioms[unitId % khaniIdioms.length];
      exercises.push({
        id: `u-${unitId}-idiom`,
        type: "intro",
        question: {
          Sorani: "پەندی کوردی و دانایی",
          Kurmanji: "Pendê Kurdî u Zanabûn",
        },
        answer: "",
        hint: `"${idiom.phrase}"`,
    ...
    exercises.push({
      ...
      type: "intro",
      question: {
        Sorani: "بنەماکانی ڕستەسازی (SOV)",
        Kurmanji: "Bingehên Hevokê (SOV)",
      },
      answer: "",
    ...
    exercises.push(
      ...current.exercises.map((ex: any, idx: number) => ({
        id: `u-${unitId}-sov-ex-${idx}`,
        type: "construct" as const,
        question: {
          Sorani: "ڕستەکە ڕێکبخە:",
          Kurmanji: "Hevokê saz bike:",
        },
        answer: ex.a,
        options: seededShuffle(ex.options, ex.a),
        hint: isSorani ? `بکەر + بەرکار + کردار` : `Biker + Berkar + Lêker`,
        tip: isSorani
  ...
  return unitDefinitions.map((unitDef) => {
    const unitWords = academicDict.filter((w) =>
      unitDef.categories.includes(w.category as string),
    );
    const lessons: Lesson[] = [];
    const questionsPerLesson = 8;
    const lessonCount = Math.max(
      1,
      Math.ceil(unitWords.length / questionsPerLesson),
    );

    for (let l = 0; l < lessonCount; l++) {
      const exercises: Exercise[] = [];
      const currentLessonWords = unitWords.slice(
        l * questionsPerLesson,
        (l + 1) * questionsPerLesson,
      );
      ...
      const khaniQuotes = [
        {
          s: "فێربوونی زمان، کلیلی دەرگای ئازادییە.",
          k: "Fêrbûna ziman, kilîta deriyê azadiyê ye.",
        },
        {
          s: "نەتەوەیەک زمانی خۆی پاراست، ناسنامەی خۆی پاراستووە.",
          k: "Neteweyek ger zimanê xwe biparêze, nasnameya xwe parastiye.",
        },
        {
          s: "هەر وشەیەک دەیزانیت، پەنجەرەیەکە ڕووەو ڕووناکی.",
          k: "Her peyva ku tu dizanî, pencereyek e ber bi ronahiyê ve.",
        },
        { s: "خوێندن و زانین هێزی ئێمەن.", k: "Xwendin u zanîn hêza me ne." },
      ];
      const randomQuote = khaniQuotes[(unitDef.id + l) % khaniQuotes.length];

      exercises.push({
        id: `u-${unitDef.id}-l-${l}-intro`,
        type: "intro",
        question: {
          Sorani: `بەخێربێیت بۆ وانەی ${unitDef.name}`,
          Kurmanji: `Bi xer hatî dersa ${unitDef.name}`,
        },
        answer: "",
        hint: isSorani
      ...
      currentLessonWords.forEach((item, i) => {
        ...
        const getDistractors = (
          ...
          const seed = getHash(current + (category || ""));

          // Try to find same category first
          if (category) {
            const sameCat = words.filter((w) => w.category === category);
            if (sameCat.length > 2) {
              const startIdx = seed % sameCat.length;
              for (
                let k = 0;
                k < sameCat.length && distractors.length < count;
                k++
              ) {
                const word = sameCat[(startIdx + k) % sameCat.length];
          ...
          // Fill rest with pseudo-random (stable)
          const startIdx = (seed + 13) % words.length;
        ...
        const getDialectHint = (item: any) => {
          ...
          if (cat === "Body") {
            return isSorani
              ...
              : "Naskirina endamên lêşێ mirovan agahiyeke bingehîn e ji bo tenduristî û têkiliyên rojane.";
          }
          if (cat === "Places") {
            return isSorani
        ...
        const typeRoll = (i + l) % 5;
        if (typeRoll === 0) {
          exercises.push({
            id: `u-${unitDef.id}-l-${l}-ex-${i}-s`,
            type: "select-correct",
            question: {
              Sorani: `واتای وشەی "${targetWord}" لە ${getDialectName(target)}دا چییە؟`,
              Kurmanji: `Wateya peyva "${targetWord}" di ${getDialectName(target)} de çi ye?`,
            },
            ...
            tip: (item as any).tip
              ? (item as any).tip[interfaceLang] || (item as any).tip["Sorani"]
              : undefined,
            originalItem: item,
          });
        } else if (typeRoll === 1) {
          exercises.push({
            ...
            type: "translation",
            question: {
              Sorani: `بیکە بە ${getDialectName(interfaceLang)}: "${targetWord}"`,
              Kurmanji: `Bike ${getDialectName(interfaceLang)}: "${targetWord}"`,
            },
            answer: interfaceWord,
            options: seededShuffle(
              [
          ...
          // Improved randomization for match pairs (Stable)
          const rawPool = unitWords.length >= 4 ? unitWords : academicDict;
          // Filter out dictionary words missing either dialect or interface translations
          const pool = rawPool.filter(
            (p) =>
              p[dialectKey] &&
              p[interfaceKey] &&
              String(p[dialectKey]).trim() !== "" &&
              String(p[interfaceKey]).trim() !== ""
          );
          const seed = targetWord + l + i;
          const selection = seededShuffle([...pool], seed).slice(0, 4);

          const pairs = selection.map((p) => ({
            left: p[dialectKey]!,
            right: p[interfaceKey]!,
          }));
          ...
          exercises.push({
            ...
            type: "match",
            question: {
              Sorani: "وشەکان بگونجێنە:",
              Kurmanji: "Peyvan li hev bînin:",
            },
            pairs: pairs,
            options: seededShuffle(
              pairs.map((p) => p.left),
            ...
            answer: seededShuffle(
              pairs.map((p) => p.right),
              seed + "right",
            ),
            hint: isSorani
              ? "هەر وشەیەک لە لای چەپەوە لەگەڵ واتا ڕاستەکەی لە لای ڕاست ببەستەوە."
              : "Her peyva li aliyê çepê bi wateya wê ya rast a li aliyê rastê ve girê bide.",
            tip: (item as any).tip
              ? (item as any).tip[interfaceLang] || (item as any).tip["Sorani"]
              : undefined,
            originalItem: item,
          ...
          // Fill in the blank (Grammar / Context)
          const pronouns: Record<string, string[]> = {
            sorani: ["من", "تۆ", "ئەو"],
            kurmanji: ["Ez", "Tu", "Ew"],
            hawrami: ["من", "تۆ", "ئەو"],
          };
          const targetPronouns = pronouns[dialectKey] || pronouns.sorani;
          const randomPronoun =
            targetPronouns[getHash(targetWord + l + i) % targetPronouns.length];
          ...
          const isAdj = ["Adjectives", "Colors"].includes(category);
          const isFamily = category === "Family";

          if (target === "Kurmanji") {
            if (isVerb) {
              const infinitive = targetWord.toLowerCase();
              // Verbs in Kurmanji usually take -ê or -î suffix after 'ji'
              const suffix = infinitive.endsWith("n") ? "ê" : "";
              qStr = `${randomPronoun} ji ${infinitive}${suffix} hez ____`;
              correct =
                randomPronoun === "Ez"
                  ? "dikim"
                  : randomPronoun === "Tu"
                    ? "dikî"
                    : "dike";
              distractors = ["dikin", "dixwim", "diçim"];
            } else if (isAdj) {
              const copula =
                randomPronoun === "Ez"
                  ? "im"
                  : randomPronoun === "Tu"
                    ? "î"
                    : "e";
              qStr = `${randomPronoun} ${targetWord.toLowerCase()} ____`;
              correct = copula;
              distractors =
                copula === "im" ? ["î", "e", "in"] : ["im", "in", "me"];
            } else if (isFamily) {
              // In Kurmanji, family members often have possessive pronouns used differently
              // e.g., 'Ev birayê min e' (This is my brother)
              const isMale =
                ["Brother", "Father", "Uncle", "Grandfather"].some((f) =>
                  targetWord.toLowerCase().includes(f.toLowerCase()),
                ) ||
                ["bira", "bav", "ap", "meta", "xal"].some((f) =>
                  targetWord.toLowerCase().includes(f),
                );
              const suffix = isMale ? "ê" : "a";
              qStr = `Ev ${targetWord.toLowerCase()}${suffix} ____ e`;
              correct = "min";
              distractors = ["te", "wî", "wê"];
            } else {
              // Nouns: Correct copula based on vowel/consonant
              const endsInVowel = /[aeêiîouû]$/i.test(targetWord);
              const copula = endsInVowel ? "ye" : "e";
              qStr = `Ev ____ ${copula}`;
              correct = targetWord.toLowerCase();
            ...
            // Sorani
            if (isVerb) {
              qStr = `${randomPronoun} حەزم لە ${targetWord}____`;
              correct = "ە";
              distractors = ["م", "مان", "یان"];
            } else if (isAdj) {
              const suffix =
                randomPronoun === "من"
                  ? "م"
                  : randomPronoun === "تۆ"
                    ? "یت"
                    : "ە";
              qStr = `${randomPronoun} ${targetWord}____`;
              correct = suffix;
              distractors = ["ین", "م", "مان"];
            } else if (isFamily) {
              qStr = `ئەمە ${targetWord}____ە`;
              correct = "م";
              distractors = ["ت", "یان", "مان"];
            } else {
              // Nouns: Correct copula (e for consonant, ye for vowel)
              const endsInVowel = /[اوەێیی]$/.test(targetWord);
              const copula = endsInVowel ? "یە" : "ە";
              qStr = `ئەمە ____${copula}`;
              correct = targetWord;
              distractors = getDistractors(3, targetWord, dialectKey, category);
            }
          }

          exercises.push({
            id: `u-${unitDef.id}-l-${l}-ex-${i}-f`,
            type: "fill-blank",
            question: {
              Sorani: `بۆشاییەکە پڕ بکەرەوە: "${qStr}"`,
              Kurmanji: `Valahiyê dagire: "${qStr}"`,
            },
            answer: correct,
            options: seededShuffle(
              [correct, ...distractors],
            ...
            hint: isSorani
              ? "بایەخ بە کۆتایی ڕستەکە بدە بۆ دۆزینەوەی کردار یان هاوەڵکار"
              : "Li dawiya hevokê binêre da ku lêkerê rast bibînî.",
            originalItem: item,
          });
        } else {
          // Improved Dialect-Aware Sentence Construction Logic
          const category = item.category || "";
          ...
          const targetPronouns = pronouns[dialectKey] || pronouns.sorani;
          const pronounSeed = getHash(targetWord + l + i + "pronoun");
          const randomPronoun =
            targetPronouns[pronounSeed % targetPronouns.length];
          ...
          if (target === "Kurmanji") {
            if (isVerb) {
              answerStr = `Ez ji xwendinê hez dikim`; // Dynamic based on word later? No, fixed for sense
              if (targetWord.toLowerCase().includes("xwen")) {
                answerStr = `Ez pirtûkan dixwînim`;
                words = ["Ez", "pirtûkan", "dixwînim"];
              } else {
                answerStr = `Ez ji ${targetWord.toLowerCase()} hez dikim`;
                words = ["Ez", "ji", targetWord.toLowerCase(), "hez", "dikim"];
              }
            } else if (isFood) {
              answerStr = `Ez ${targetWord.toLowerCase()} dixwîm`;
              words = ["Ez", targetWord.toLowerCase(), "dixwîm"];
            } else if (isFamily) {
              answerStr = `Ez hez ji ${targetWord.toLowerCase()}ê xwe dikim`;
              words = [
                "Ez",
                "hez",
                "ji",
                `${targetWord.toLowerCase()}ê`,
                "xwe",
                "dikim",
              ];
            } else if (isBody) {
              answerStr = `${targetWord.toLowerCase()}ê min diêşe`;
              words = [`${targetWord.toLowerCase()}ê`, "min", "diêşe"];
            } else if (isAdj) {
              const copula =
                randomPronoun === "Ez"
                  ? "im"
                  : randomPronoun === "Tu"
                    ? "î"
                    : "e";
              answerStr = `${randomPronoun} ${targetWord.toLowerCase()} ${copula}`;
              words = [randomPronoun, targetWord.toLowerCase(), copula];
            } else if (isPlace) {
              answerStr = `Ez li ${targetWord.toLowerCase()} im`;
              words = ["Ez", "li", targetWord.toLowerCase(), "im"];
            } else {
              const endsInVowel = /[aeêiîouû]$/i.test(targetWord);
              const copula = endsInVowel ? "ye" : "e";
              answerStr = `Ev ${targetWord.toLowerCase()} ${copula}`;
              words = ["Ev", targetWord.toLowerCase(), copula];
            }
          } else {
            // Sorani Default
            if (isVerb) {
              if (targetWord.includes("خوێن")) {
                answerStr = "من کتێب دەخوێنمەوە";
                words = ["من", "کتێب", "دەخوێنمەوە"];
              } else {
                answerStr = `من حەزم لە ${targetWord}ە`;
                words = ["من", "حەزم", "لە", `${targetWord}ە`];
              }
            } else if (isFood) {
              answerStr = `من ${targetWord} دەخۆم`;
              words = ["من", targetWord, "دەخۆم"];
            } else if (isFamily) {
              // In Sorani, for family we often say "I love my father"
              // Using one 'm' as the user suggested: "من باوکم خۆشدەوێت"
              answerStr = `من ${targetWord}م خۆشدەوێت`;
              words = ["من", `${targetWord}م`, "خۆشدەوێت"];
            } else if (isBody) {
              answerStr = `${targetWord}م دەهێشێت`;
              words = [`${targetWord}م`, "دەهێشێت"];
            } else if (isAdj) {
              let copula = "";
              if (randomPronoun === "من") copula = "م";
              else if (randomPronoun === "تۆ") copula = "یت";
              else copula = /[اەووێی]$/.test(targetWord) ? "یە" : "ە";
              answerStr = `${randomPronoun} ${targetWord}${copula}`;
              words = [randomPronoun, `${targetWord}${copula}`];
            } else if (isPlace) {
              answerStr = `من لە ${targetWord}م`;
              words = ["من", "لە", `${targetWord}م`];
            } else {
          ...
          if (words.length === 0) {
            exercises.push({
              id: `u-${unitDef.id}-l-${l}-ex-${i}-c-f`,
              type: "translation",
              question: {
                Sorani: `ئەم وشەیە وەربگێڕە: "${targetWord}"`,
                Kurmanji: `Vê peyvê wergerîne: "${targetWord}"`,
              },
              answer: interfaceWord,
              options: seededShuffle(
                [
                  interfaceWord,
                  ...getDistractors(3, interfaceWord, interfaceKey, category),
                ],
                targetWord + "c-f",
              ),
              hint: getDialectHint(item),
          ...
          const extensions = getDistractors(2, "", dialectKey, category);
          const finalOptions = seededShuffle(
            [...words, ...extensions],
          ...
          exercises.push({
            ...
            hint: isSorani
              ? "ڕستەکە بە بکەر دەست پێ دەکات و بە کردار کۆتایی دێت."
              : "Hevok bi biker dest pê dike û bi lêker diqede.",
            tip: (item as any).tip
              ? (item as any).tip[interfaceLang] || (item as any).tip["Sorani"]
              : undefined,
            originalItem: item,
    ...
    // Apply lesson overrides if any
    const finalLessons = lessons.map((lesson) => {
      const overrideKey = `${target}_${lesson.id}`;
      let override = (appConfig?.lessonOverrides as any)?.[overrideKey];
      if (!override && target === "Sorani") {
        override = (appConfig?.lessonOverrides as any)?.[lesson.id];
      }
      return override ? { ...lesson, ...override } : lesson;
    });

    if (unitDef.id === 1) {
      const sovLesson = getSOVLesson(target, interfaceLang, unitDef.id);
      const sovOverrideKey = `${target}_${sovLesson.id}`;
      let override = (appConfig?.lessonOverrides as any)?.[sovOverrideKey];
      if (!override && target === "Sorani") {
        override = (appConfig?.lessonOverrides as any)?.[sovLesson.id];
      }
      finalLessons.push(override ? { ...sovLesson, ...override } : sovLesson);
    }
    ...
    if (this.state.hasError) {
      ...
      try {
        ...
        if (parsed.isQuota) {
          ...
          errorMessage =
            "ژمارەی هەوڵە ڕێگەپێدراوەکانی ڕۆژانەی پڕۆژەکە تەواو بووە. تکایە بەیانی سەردانمان بکەرەوە.";
        }
      } catch (e) {}

      return (
        <div
          className="min-h-screen theme-bg-page flex items-center justify-center p-4 text-center text-white"
          style={{ backgroundColor: "#05010a" }}
        >
          <div className="max-w-md p-4 bg-slate-900 border border-white/10 rounded-[1.25rem]">
            <div className="w-14 h-14 bg-red-500/20 rounded-full flex items-center justify-center mx-auto mb-6">
              {isQuota ? (
            ...
            <h2 className="text-2xl font-black mb-4">
              {isQuota ? "کۆتای ڕۆژانە" : "هەڵەیەک ڕوویدا"}
            </h2>
            <p className="opacity-60 mb-8 font-medium italic">{errorMessage}</p>
            {isQuota ? (
              <p className="text-xs opacity-40 mb-8">
                Ev sepan li ser plana belaş a Firebase e. Kotaya rojane qediya
                ye, sibê dîsa were.
              </p>
            ) : null}
            <button
...
const SmartImage = ({ src, alt, className, style, ...props }: any) => {
  ...
  useEffect(() => {
    ...
    const loadRealSrc = async () => {
      ...
      // Indirect URLs check
      if (
        (finalUrl.includes("postimg.cc") &&
          !finalUrl.includes("i.postimg.cc")) ||
        (finalUrl.includes("ibb.co") && !finalUrl.includes("i.ibb.co")) ||
        (!finalUrl.match(/\.(jpeg|jpg|gif|png|webp|svg)$/i) &&
          finalUrl.startsWith("http"))
      ) {
        try {
          // Use Microlink free API to extract image
          const res = await fetch(
            `https://api.microlink.io/?url=${encodeURIComponent(finalUrl)}`,
          );
          const data = await res.json();
          if (active && data?.data?.image?.url) {
            setActualSrc(data.data.image.url);
          } else {
            setActualSrc(finalUrl);
          }
        } catch (e) {
          if (active) setActualSrc(finalUrl);
        }
      } else {
        setActualSrc(finalUrl);
      }
    ...
    <img
      ...
      onError={(e) => {
        ...
        if (!target.src.includes("placehold.co")) {
          target.src =
            "https://placehold.co/600x400/1a1a2e/ffffff?text=Image+URL+Error:\\nپێویستە+لینکەکە+ڕاستەوخۆ+بێت\\n(Direct+Link)";
          target.classList.add("opacity-80", "grayscale");
        }
      }}
      {...props}
    />
...
const AdminSchoolTopicEditor = ({
  ...
  const handleSave = async () => {
    ...
    // Important: we match by the ORIGINAL title mapped from `topic`, so if they rename it still updates
    const globalIdx = rawList.findIndex((rt: any) => rt.title === topic.title);
    
    if (globalIdx === -1) {
      console.warn("Topic not found in global state for saving.");
      setIsSaving(false);
      return;
    }

    const list = [...rawList];
    list[globalIdx] = { ...localTopic };
    // Remove the dialect property before saving, as it's injected by the client map
    delete list[globalIdx].dialect;

    try {
      await setDoc(
        doc(db, "app", "global", "dialects", dialectKey),
        { topics: cleanObject(list) },
        { merge: true }
      );
      // Play success visual/audio feedback here if needed
    } catch (err) {
  ...
  const updateField = (field: string, val: string) => {
    setLocalTopic((prev: any) => ({ ...prev, [field]: val }));
  };

  return (
    <div className="bg-white/5 rounded-3xl border theme-border-soft p-5">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex gap-2 ml-4">
          <button
            onClick={handleSave}
            disabled={isSaving}
            className={`px-4 py-2 flex-shrink-0 flex items-center gap-2 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${
              isSaving
          ...
          >
            <Save size={14} />
            {isSaving ? "..." : interfaceLang === "Sorani" ? "پاشەکەوت" : "Save"}
          </button>
          <button
            onClick={handleDelete}
            className="p-2 flex-shrink-0 bg-rose-500/10 rounded-xl text-rose-500 hover:bg-rose-500 hover:text-white transition-all flex items-center"
          >
            <Trash2 size={14} />
          </button>
        </div>
      ...
      <textarea
        value={localTopic.content || ""}
        onChange={(e) => updateField("content", e.target.value)}
        className="w-full bg-black/20 border border-white/5 rounded-2xl p-4 min-h-[150px] text-xs text-white/70 outline-none focus:border-white/20 transition-all font-mono whitespace-pre-wrap"
      ...
      <div className="mt-4 grid grid-cols-2 gap-4">
        <div className="space-y-1">
          <span className="text-[8px] font-black uppercase tracking-tighter opacity-40 ml-2">
            Type/Format (جۆری بابەت)
          </span>
          <input
            value={localTopic.format || ""}
            onChange={(e) => updateField("format", e.target.value)}
            className="w-full bg-black/20 border border-white/5 rounded-xl px-3 py-2 text-[10px] text-white outline-none focus:border-white/20"
          />
        </div>
        <div className="space-y-1">
          <span className="text-[8px] font-black uppercase tracking-tighter opacity-40 ml-2">
            Category (Ziman/Şêwezar)
          </span>
          <input
            value={localTopic.category || ""}
...
export default function App() {
  ...
  const [interfaceLang, setInterfaceLang] = useState<InterfaceLang>(
    () =>
      (localStorage.getItem("ferga_interface") as InterfaceLang) || "Sorani",
  );
  const [targetDialect, setTargetDialect] = useState<TargetDialect>(
    () => (localStorage.getItem("ferga_target") as TargetDialect) || "Sorani",
  );
  ...
  const [adminSubTab, setAdminSubTab] = useState<
    "general" | "labels" | "tabs" | "school" | "dictionary"
  >("general");
  const [adminSelectedLessonId, setAdminSelectedLessonId] = useState<
    string | null
  >(null);
  ...
  const [activeAdminDialect, setActiveAdminDialect] = useState<string | null>(
    null,
  );
  const [activeAdminLessonDialect, setActiveAdminLessonDialect] =
    useState<TargetDialect>(
      () => (localStorage.getItem("ferga_target") as TargetDialect) || "Sorani",
    );
  ...
  const isAdminUser = useMemo(() => {
    return (
      user?.uid === "2jRQzIcFHvcK0YcnqkfdmQ4rPPk2" ||
      user?.email === "ibrahimrebwar2026@gmail.com" ||
      user?.email === "ibrahimrebwr2026@gmail.com"
    );
  }, [user]);

  const migrationAttempted = useRef(false);

  // Load Global App Config and split content
  useEffect(() => {
    let globalUnsub: (() => void) | null = null;
    let schoolUnsub: (() => void) | null = null;
    let overridesUnsub: (() => void) | null = null;
    let dictionaryUnsub: (() => void) | null = null;
    ...
    const loadData = () => {
      ...
      globalUnsub = onSnapshot(
        ...
        (err) => {
          handleFirestoreError(err, "get", "app/global", user);
        },
      );

      schoolUnsub = onSnapshot(
        collection(db, "app", "global", "dialects"),
        (snap) => {
          const school: any = {};
          snap.docs.forEach((d) => (school[d.id] = d.data().topics || []));
          currentSchool = school;
          updateConfig();
        },
        (err) => {
          handleFirestoreError(err, "list", "app/global/dialects", user);
        },
      );

      overridesUnsub = onSnapshot(
        collection(db, "app", "global", "overrides"),
        (snap) => {
          const overrides: any = {};
        ...
        (err) => {
          handleFirestoreError(err, "list", "app/global/overrides", user);
        },
      );

      dictionaryUnsub = onSnapshot(
        collection(db, "app", "global", "dictionary"),
        (snap) => {
          const dict: any[] = snap.docs.map((d) => ({ ...d.data(), id: d.id }));
          currentDict = dict;
          updateConfig();
        },
        (err) => {
          handleFirestoreError(err, "list", "app/global/dictionary", user);
        },
      );
    };
  ...
  const saveGlobalConfig = async (newConfig: any) => {
    ...
    try {
      ...
      // Handle subcollections if they accidentally passed as large maps
      if (cleanedConfig.dialectSchoolContent) {
        for (const [dialect, topics] of Object.entries(
          cleanedConfig.dialectSchoolContent,
        )) {
          await setDoc(
            doc(db, "app", "global", "dialects", dialect),
            { topics: cleanObject(topics) },
            { merge: true },
          );
        }
        cleanedConfig.dialectSchoolContent = deleteField();
      }

      if (cleanedConfig.lessonOverrides) {
        for (const [lessonId, data] of Object.entries(
          cleanedConfig.lessonOverrides,
        )) {
          await setDoc(
            doc(db, "app", "global", "overrides", lessonId),
            cleanObject(data),
            { merge: true },
          );
        }
        cleanedConfig.lessonOverrides = deleteField();
      }

      if (cleanedConfig.customDictionary) {
        for (const word of cleanedConfig.customDictionary as any[]) {
          const id = String(word.id || word.sorani).replace(/\//g, "_");
          await setDoc(
        ...
        cleanedConfig.customDictionary = deleteField();
      }

      if (Object.keys(cleanedConfig).length > 0) {
        await updateDoc(doc(db, "app", "global"), cleanedConfig);
      }
    } catch (err: any) {
  ...
  const saveLessonOverride = async (
    lessonId: string | number,
    exercises: Exercise[],
  ) => {
    if (!isAdminUser) return;
    const lessonIdStr = `${activeAdminLessonDialect}_${lessonId}`;
    try {
      // Ensure exercises are completely pure JSON and strip originalItem to prevent duplication
      const pureExercises = JSON.parse(JSON.stringify(exercises)).map(
        (ex: any) => {
          delete ex.originalItem;
          return ex;
        },
      );
      const docRef = doc(db, "app", "global", "overrides", lessonIdStr);
      await setDoc(docRef, { exercises: pureExercises }, { merge: true });
      setToast({
        message: "وانە نوێکرایەوە / Ders hat nûkirin",
        type: "success",
      });
    } catch (err: any) {
      setToast({ message: "Error: " + err.message, type: "error" });
      handleFirestoreError(
        err,
        "write",
        `app/global/overrides/${lessonIdStr}`,
        user,
      );
    }
  };

  const saveDictionaryWord = async (originalWord: any, updatedFields: any) => {
    if (!isAdminUser) return;
    const wordId = String(originalWord.id || originalWord.sorani);
    const sanitizedId = wordId.replace(/\//g, "_");
    try {
      await setDoc(
        doc(db, "app", "global", "dictionary", sanitizedId),
        {
          ...cleanObject(originalWord),
          ...cleanObject(updatedFields),
          id: wordId,
        },
        { merge: true },
      );
      setToast({ message: "پاشەکەوت کرا / Qeyd bû", type: "success" });
    } catch (err: any) {
      setToast({ message: "Error: " + err.message, type: "error" });
      handleFirestoreError(
        err,
        "write",
        `app/global/dictionary/${sanitizedId}`,
        user,
      );
    }
  };

  const deleteDictionaryWord = async (word: any) => {
    if (!isAdminUser) return;
    const wordId = String(word.id || word.sorani);
    const sanitizedId = wordId.replace(/\//g, "_");
    try {
      await deleteDoc(doc(db, "app", "global", "dictionary", sanitizedId));
      setToast({ message: "سڕایەوە / Hat jêbirin", type: "success" });
    } catch (err) {
      handleFirestoreError(
        err,
        "write",
  ...
  const activeTabsList = useMemo(() => {
    ...
    if (isAdminUser) {
      defaultTabs.push({
        id: "admin",
        icon: Crown,
        label: interfaceLang === "Sorani" ? "ئەدمین" : "Admin",
      });
    }

    if (appConfig?.extraTabs) {
      const extra = appConfig.extraTabs.map((t: any, i: number) => ({
        ...t,
        id: t.id ? t.id : `extra-${i}`,
      }));
      // Filter out duplicate IDs
      const allTabs = [...defaultTabs, ...extra];
      const uniqueTabs = [];
  ...
  const [croppingType, setCroppingType] = useState<"avatar" | "banner">(
    "avatar",
  );
  ...
  const [xp, setXp] = useState(
    () => Number(localStorage.getItem("ferga_xp")) || 0,
  );
  const [lives, setLives] = useState(5);
  const [streak, setStreak] = useState(
    () => Number(localStorage.getItem("ferga_streak")) || 0,
  );
  const [lastActivityDate, setLastActivityDate] = useState<string | null>(() =>
    localStorage.getItem("ferga_last_activity"),
  );
  ...
  const [dailyGoal, setDailyGoal] = useState(
    () => Number(localStorage.getItem("ferga_daily_goal")) || 2,
  );
  ...
  const [authStatus, setAuthStatus] = useState<
    "idle" | "linking" | "sent" | "success"
  >("idle");
  const [authError, setAuthError] = useState<string | null>(null);
  const [selectedArticle, setSelectedArticle] = useState<any | null>(null);
  const [activeGrammarDialectId, setActiveGrammarDialectId] =
    useState("sorani");
  const [grammarView, setGrammarView] = useState<"explore" | "compare">(
    "explore",
  );
  const [compareDialects, setCompareDialects] = useState<{
    d1: string;
    d2: string;
  }>({ d1: "sorani", d2: "kurmanji" });
  ...
  const [schoolGrouping, setSchoolGrouping] = useState<
    "dialect" | "book" | "author"
  >(() => (localStorage.getItem("ferga_school_group") as any) || "category");

  useEffect(() => {
    localStorage.setItem("ferga_school_group", schoolGrouping);
  }, [schoolGrouping]);
  const [selectedSchoolCategory, setSelectedSchoolCategory] = useState<
    string | null
  >(null);
  const [selectedSchoolTopic, setSelectedSchoolTopic] = useState<any | null>(
    null,
  );
  ...
  const allSchoolTopics = useMemo(() => {
    const all: any[] = [];
    Object.entries(appConfig?.dialectSchoolContent || {}).forEach(
      ([dialect, topics]: any) => {
  ...
  useEffect(() => {
    localStorage.setItem(
      "ferga_school_read",
      JSON.stringify(Array.from(schoolTopicReadStatus)),
    );
  ...
  const [readingFontSize, setReadingFontSize] = useState(
    () => Number(localStorage.getItem("ferga_school_font_size")) || 16,
  );
  const [schoolReadingBg, setSchoolReadingBg] = useState(
    () => localStorage.getItem("ferga_school_bg") || "transparent",
  );
  const [schoolReadingColor, setSchoolReadingColor] = useState(
    () => localStorage.getItem("ferga_school_color") || "rgba(255,255,255,0.8)",
  );
  ...
  const [fontSize, setFontSize] = useState(
    () => localStorage.getItem("ferga_font_size") || "medium",
  );
  ...
  const [editingDialectKey, setEditingDialectKey] = useState<{
    original: string;
    current: string;
  } | null>(null);
  ...
  useEffect(() => {
    ...
    correctAudio.current = new Audio(
      "https://www.soundjay.com/misc/sounds/bell-ring-01.mp3",
    );
    wrongAudio.current = new Audio(
      "https://www.soundjay.com/misc/sounds/fail-trumpet-01.mp3",
    );

    [clickAudio.current, correctAudio.current, wrongAudio.current].forEach(
      (a) => {
        if (a) {
          a.volume = 1.0;
          a.preload = "auto";
  ...
  const checkAnswer = () => {
    ...
    const isCorrect = isAnswerCorrect(currentEx, {
      selectedOption,
      translationWords: usedOptionIndices.map(
        (i) => currentEx.options?.[i] || "",
      ),
      completedPairs,
    });
    ...
    if (isCorrect) {
      ...
      const points = 1; // 1 XP for correct answers
      ...
      const newMistakes = mistakesOnCurrent + 1;
      setMistakesOnCurrent(newMistakes);
  ...
  const nextExercise = () => {
    ...
    if (!isReviewPhase) {
      const isLastOfInitial = exerciseIndex >= activeLesson.exercises.length - 1;
      if (!isLastOfInitial) {
  ...
  const finishLesson = () => {
    ...
    const { streak: newStreak, lastActivity: lastDate } = calculateNextStreak(
      streak,
      lastActivityDate,
      today,
    );
    ...
    // Solve React stale closure and sync bugs completely by using functional updaters
    setXp((currentXp) => {
      const finalXp = currentXp + 2; // +2 XP Lesson Completion reward
      localStorage.setItem("ferga_xp", String(finalXp));
      localStorage.setItem(
        "ferga_progress",
  ...
  const [feedback, setFeedback] = useState<"correct" | "wrong" | "wrong_first" | null>(null);
  ...
  const [homeMascotTip, setHomeMascotTip] = useState<string | null>(() => {
    const tips =
      interfaceLang === "Sorani" ? kurdishTips.Sorani : kurdishTips.Kurmanji;
    return tips[Math.floor(Math.random() * tips.length)];
  });
  const [homeMascotMood, setHomeMascotMood] = useState<
    "happy" | "thinking" | "wink"
  >("happy");
    ...
    const unsub = onAuthStateChanged(auth, async (u) => {
      ...
      if (u) {
        ...
        try {
          ...
          // Pre-fetch local storage stats for robust, fair, zero-reset merging
          const localXp = Number(localStorage.getItem("ferga_xp")) || 0;
          const localStreak = Number(localStorage.getItem("ferga_streak")) || 0;
          const localProgress = (() => {
            try {
              return JSON.parse(localStorage.getItem("ferga_progress") || "[]");
            } catch {
              return [];
            }
          })();
          ...
          if (docSnap.exists()) {
            ...
            // Bilaterally merge XP fairly so offline play or higher scores are kept
            const dbXp = data.xp || 0;
            const finalXp = Math.max(localXp, dbXp);
            setXp(finalXp);
            localStorage.setItem("ferga_xp", String(finalXp));
            
            // Bilaterally merge streak
            const dbStreak = data.streak || 0;
            const finalStreak = Math.max(localStreak, dbStreak);
            setStreak(finalStreak);
            localStorage.setItem("ferga_streak", String(finalStreak));

            // Bilaterally merge learned lessons (progress)
            const dbProgress = data.learnedLessons || [];
            const mergedProgressSet = new Set([...localProgress, ...dbProgress]);
            setLearnedLessons(mergedProgressSet);
            localStorage.setItem("ferga_progress", JSON.stringify(Array.from(mergedProgressSet)));
            ...
            // If local data had more progress/rewards than cloud, sync it to cloud!
            const cloudUpdates: any = {};
            if (finalXp > dbXp) cloudUpdates.xp = finalXp;
            if (finalStreak > dbStreak) cloudUpdates.streak = finalStreak;
            if (mergedProgressSet.size > dbProgress.length) {
              cloudUpdates.learnedLessons = Array.from(mergedProgressSet);
            }

            // Load Flashcards
            const fcRef = collection(db, "users", u.uid, "flashcards");
            const fcSnap = await getDocs(fcRef);
            const fcData = fcSnap.docs.map(
              (doc) => ({ id: doc.id, ...doc.data() }) as Flashcard,
            );
            setFlashcards(fcData);

            // Update profile info if Google data is updated
            ...
            if (cleanAuthName && cleanDbName !== cleanAuthName)
              cloudUpdates.displayName = cleanAuthName;
            if (cleanAuthPhoto && cleanDbPhoto !== cleanAuthPhoto)
              cloudUpdates.photoURL = cleanAuthPhoto;

            if (
              Object.keys(cloudUpdates).length > 0 &&
              !sessionStorage.getItem("ferga_quota_exceeded")
            ) {
              updateDoc(docRef, cloudUpdates).catch((e) =>
                handleFirestoreError(e, "update", `users/${u.uid}`, u),
              );
            }
          } else {
            // Initialize new user document with their local play progress intact!
            try {
              await setDoc(docRef, {
                xp: localXp,
                learnedLessons: localProgress,
                email: u.email,
                streak: localStreak,
                lastActivity: localStorage.getItem("ferga_last_activity") || "",
                lessonsToday: Number(localStorage.getItem("ferga_lessons_today")) || 0,
                dailyGoal: 2,
                achievements: [],
              ...
              setXp(localXp);
              setLearnedLessons(new Set(localProgress));
              setStreak(localStreak);
              setLessonsToday(Number(localStorage.getItem("ferga_lessons_today")) || 0);
              setLastActivityDate(localStorage.getItem("ferga_last_activity") || null);
            } catch (e) {
  ...
  const dialectKey = targetDialect.toLowerCase() as
    | "sorani"
    | "kurmanji"
    | "hawrami"
    | "zazaki"
  ...
  const units = useMemo(
    () => getUnitData(targetDialect, interfaceLang, appConfig),
    [targetDialect, interfaceLang, appConfig],
  );

  const activeFlashcards = useMemo(() => {
    if (flashcards.length > 0) return flashcards;
    
    const dialectKey = targetDialect.toLowerCase();
    const interfaceKey = interfaceLang.toLowerCase();
    
    const daySeed = new Date().toDateString();
    
    return stableShuffle([...academicDict], daySeed)
      .filter((w: any) => w[dialectKey] && w[interfaceKey] && String(w[dialectKey]).trim() !== "")
      .slice(0, 50)
      .map((w: any, idx: number) => ({
        id: `auto-${idx}`,
        front: w[interfaceKey] || "",
        back: w[dialectKey] || "",
        example: w[`${dialectKey}_example`] || w.category || "",
        category: w.category || "Vocabulary",
        streak: 0
      } as Flashcard));
  }, [flashcards, academicDict, targetDialect, interfaceLang]);

  const adminUnits = useMemo(
    () => getUnitData(activeAdminLessonDialect, interfaceLang, appConfig),
    [activeAdminLessonDialect, interfaceLang, appConfig],
  );
  const activeLesson = useMemo(() => {
    ...
    return found || null;
  }, [activeLessonId, units, adminUnits]);
  ...
  const handleDialectChange = (d: TargetDialect) => {
    ...
    if (user && !sessionStorage.getItem("ferga_quota_exceeded")) {
      updateDoc(doc(db, "users", user.uid), { targetDialect: d }).catch((e) =>
        handleFirestoreError(e, "update", `users/${user.uid}`, user),
      );
    }
  ...
  const handleInterfaceChange = (i: InterfaceLang) => {
    ...
    if (user && !sessionStorage.getItem("ferga_quota_exceeded")) {
      updateDoc(doc(db, "users", user.uid), { interfaceLang: i }).catch((e) =>
        handleFirestoreError(e, "update", `users/${user.uid}`, user),
      );
    }
  ...
  const onCropSave = async () => {
    if (imageToCrop && croppedAreaPixels && auth.currentUser) {
      try {
        setIsReadingFile(true);
        ...
        if (croppingType === "avatar") {
          const downloadUrl = await uploadImageToStorage(
            croppedImage,
            `users/${userId}/avatar_${Date.now()}.jpg`,
          );
          setEditPhotoURL(downloadUrl);
        } else {
          const downloadUrl = await uploadImageToStorage(
            croppedImage,
            `users/${userId}/banner_${Date.now()}.jpg`,
          );
          setEditBannerUrl(downloadUrl);
        }
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (
        ...
        e.message?.includes("storage is inaccessible")
      ) {
        setAuthError(
          isRTL
        ...
        setAuthError(
          isRTL
            ? "پۆپ-ئەپ بلۆک کراوە، بێزەحمەت ڕێگەی پێ بدە یان لە سەفاری بەکاری بهێنە."
            : "Popup hate astengkirin, ji kerema xwe destûr bide an di Safari de bi kar bîne.",
        );
        ...
        setAuthError(
          isRTL
            ...
            : "Şaşiyek di dema têketinê de derket.",
        );
      }
    }
  };
  ...
  const handleMatchSelect = (val: string, side: "left" | "right") => {
    ...
    if (newPairs.left && newPairs.right) {
      const currentEx = activeLesson!.exercises[exerciseIndex];
      const match = currentEx.pairs?.find(
  ...
  if (!user) {
    return (
      <ErrorBoundary>
        <div className="min-h-screen theme-bg-page theme-text flex items-center justify-center p-4 font-sans relative overflow-hidden">
          <ThemeInjected dialect={targetDialect} mode={themeMode} />
          <BackgroundBlobs dialect={targetDialect} />
          <div className="absolute inset-0 z-0">
            <KurdistanMap className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[120%] h-[120%] opacity-[0.03]" />
          </div>

          <div className="max-w-md w-full text-center p-4 theme-card rounded-[1.5rem] relative z-10 shadow-2xl">
            <motion.div
            ...
            <h1 className="text-xl md:text-2xl font-black mb-1 tracking-tighter theme-text leading-tight font-afarin1">
              {labels.title}
            </h1>
            <div className="flex items-center justify-center gap-4 mb-10 opacity-40">
              <div className="h-px bg-white/10 flex-1" />
              <p className="text-[9px] font-bold theme-primary tracking-[0.4em] uppercase">
                Language Academy
              </p>
              <div className="h-px bg-white/10 flex-1" />
            </div>
            ...
            <p className="text-white/30 mb-6 text-base font-medium">
              {labels.loginTitle}
            </p>
            ...
            <p className="mt-8 text-[10px] font-bold text-white/20 uppercase tracking-[0.3em]">
              {interfaceLang === "Sorani"
                ? "فێرگەی خانی: زمانە کوردییەکان"
                : "Fêrga Xanî: Zimanên Kurdî"}
            </p>
          </div>
        </div>
      </ErrorBoundary>
  ...
  if (onboardingStage !== "complete") {
    return (
      <ErrorBoundary>
        <div className="min-h-screen theme-bg-page theme-text flex items-center justify-center p-4 font-sans">
          <ThemeInjected dialect={targetDialect} mode={themeMode} />
          <BackgroundBlobs dialect={targetDialect} />
          <div className="max-w-md w-full">
            <AnimatePresence mode="wait">
              <motion.div
                ...
                className="text-center p-4 bg-white/5 backdrop-blur-3xl border border-white/10 rounded-[1.25rem] shadow-2xl"
              >
                <GraduationCap className="w-14 h-14 mx-auto mb-6 theme-primary animate-pulse" />
                <h2 className="text-2xl font-black mb-4">{labels.target}</h2>
                <p className="text-white/40 mb-8 text-sm">
                  {isRTL
                    ? "ئەو شێوەزارە هەڵبژێرە کە دەتەوێت فێری بیت"
                    : "Zimanê ku tu dixwazî hîn bibî hilbijêre"}
                </p>
  ...
  if (activeLesson && activeTab !== "admin") {
    const currentEx = activeLesson.exercises[exerciseIndex];
    return (
      <ErrorBoundary>
        <div
          className="min-h-screen theme-bg-page theme-text overflow-hidden relative font-sans"
          dir={isRTL ? "rtl" : "ltr"}
        >
          <ThemeInjected dialect={targetDialect} mode={themeMode} />
          <BackgroundBlobs dialect={targetDialect} />
            ...
            <div className="flex items-center gap-4 mb-10 shrink-0">
              ...
              <div className="flex-1 h-3 bg-white/10 rounded-full overflow-hidden border border-white/5 shadow-inner">
                <motion.div
                  initial={{ width: 0 }}
                  animate={{
                    width: `${(exerciseIndex / activeLesson.exercises.length) * 100}%`,
                  }}
            ...
            <AnimatePresence mode="wait">
              {!isLessonComplete ? (
                ...
                >
                  {currentEx.type !== "intro" && (
                    <div className="flex items-start gap-4 mb-10">
                      <div className="w-14 h-14 shrink-0 rounded-2xl border-2 border-white/20 bg-white/5 p-1 relative shadow-2xl">
                        <KhaniPoetMascot
                        ...
                        />
                        <div className="absolute -bottom-2 -right-2 bg-slate-900 text-white rounded-lg px-2 py-0.5 text-[8px] font-black uppercase">
                          خانی
                        </div>
                      </div>
                      <div className="flex-1">
                        <div className="bg-white/10 backdrop-blur-3xl border border-white/20 p-5 rounded-3xl rounded-tr-none relative mt-2 shadow-2xl">
                          {(currentEx.mediaUrl || currentEx.imageUrl) && (
                            <div className="mb-6 theme-bg-soft border theme-border-soft rounded-3xl overflow-hidden shadow-2xl p-2 relative group">
                              <div className="absolute inset-0 theme-bg-primary opacity-[0.03] pointer-events-none" />
                              {currentEx.mediaUrl &&
                              currentEx.mediaType === "video" ? (
                                <div className="rounded-[1.25rem] overflow-hidden bg-black w-full shadow-inner border border-black/50">
                                  <iframe
                                    className="w-full aspect-video border-none"
                                    src={
                                      currentEx.mediaUrl.includes(
                                        "youtube.com",
                                      ) ||
                                      currentEx.mediaUrl.includes("youtu.be")
                                        ? `https://www.youtube.com/embed/${currentEx.mediaUrl.split("/").pop()?.split("=")?.pop()}`
                                        : currentEx.mediaUrl
                                    }
                                ...
                                <div className="rounded-[1.25rem] overflow-hidden bg-black/20 w-full flex items-center justify-center p-2 border border-white/5 min-h-[160px] relative backdrop-blur-md">
                                  <SmartImage
                                    src={
                                      currentEx.imageUrl || currentEx.mediaUrl
                                    }
                                    alt="Lesson content"
                                    className="max-w-full h-auto max-h-64 object-contain drop-shadow-2xl rounded-xl transition-transform duration-500 group-hover:scale-105 relative z-10"
                                  />
                                </div>
                              )}
                            </div>
                          ...
                          <p className="text-lg md:text-xl font-black text-white leading-tight font-afarin1">
                            {typeof currentEx.question === "string"
                              ? currentEx.question
                              : (currentEx.question as any)[interfaceLang] ||
                                (currentEx.question as any)["Sorani"]}
                          </p>
                          <div
                            className={`absolute top-0 ${isRTL ? "-right-2" : "-left-2"} w-3 h-3 bg-white/10 rotate-45 border-t border-l border-white/20`}
                          />
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="flex-1">
                    {currentEx.type === "intro" && (
                      <motion.div
                        ...
                        animate={{ scale: 1, opacity: 1 }}
                        className="theme-bg-card border theme-border-soft rounded-[1.25rem] p-4 md:p-4 text-center"
                      >
                        {currentEx.imageUrl || currentEx.mediaUrl ? (
                          <div className="w-full mx-auto mb-8 relative rounded-[1.25rem] overflow-hidden bg-black/20 border border-white/5 min-h-[160px] flex items-center justify-center p-2 backdrop-blur-md shadow-2xl">
                            {currentEx.mediaUrl &&
                            currentEx.mediaType === "video" ? (
                              <div className="w-full rounded-2xl overflow-hidden bg-black shadow-inner border border-black/50">
                                <iframe
                                  className="w-full aspect-video border-none"
                                  src={
                                    currentEx.mediaUrl.includes(
                              ...
                              <SmartImage
                                src={currentEx.imageUrl || currentEx.mediaUrl}
                                alt="Intro visual"
                                className="max-w-full h-auto max-h-64 object-contain drop-shadow-2xl rounded-xl relative z-10"
                              />
                            )}
                          </div>
                        ) : (
                        ...
                        <h2 className="text-2xl font-black mb-4 theme-text font-afarin1">
                          {typeof currentEx.question === "string"
                            ? currentEx.question
                            : (currentEx.question as any)[interfaceLang] ||
                              (currentEx.question as any)["Sorani"]}
                    ...
                    {currentEx.type === "match" && (() => {
                      const leftRaw = (Array.isArray(currentEx.options) && currentEx.options.length > 0)
                        ? currentEx.options
                        : (currentEx.pairs ? stableShuffle(currentEx.pairs.map(p => p.left), (currentEx.id || "") + "-left") : []);
                        
                      const rightRaw = (Array.isArray(currentEx.answer) && currentEx.answer.length > 0)
                        ? currentEx.answer
                        : (currentEx.pairs ? stableShuffle(currentEx.pairs.map(p => p.right), (currentEx.id || "") + "-right") : []);

                      // Filter out any empty, null, or undefined values to prevent blank/invisible rows
                      const leftList = leftRaw.filter(val => val && String(val).trim() !== "");
                      const rightList = rightRaw.filter(val => val && String(val).trim() !== "");
                        ...
                        <div className="grid grid-cols-2 gap-4 md:gap-4">
                          <div className="flex flex-col gap-3">
                            {leftList.map((left, i) => (
                              <motion.button
                                key={`left-${i}`}
                                onClick={() => handleMatchSelect(left, "left")}
                                className={`p-4 rounded-xl text-sm font-bold border transition-all ${
                                  completedPairs.includes(left)
                                    ? "opacity-25 pointer-events-none theme-bg-soft theme-text-muted"
                                    : matchPairs.left === left
                                      ? "theme-bg-primary theme-border-primary text-white shadow-lg scale-105 font-black"
                                      : "theme-bg-card theme-border-soft theme-text hover:theme-bg-soft"
                                }`}
                              >
                                {left}
                          ...
                          <div className="flex flex-col gap-3">
                            {rightList.map((right, i) => (
                              <motion.button
                                key={`right-${i}`}
                                onClick={() =>
                                  handleMatchSelect(right, "right")
                                }
                                className={`p-4 rounded-xl text-sm font-bold border transition-all ${
                                  completedPairs.includes(right)
                                    ? "opacity-25 pointer-events-none theme-bg-soft theme-text-muted"
                                    : matchPairs.right === right
                                      ? "theme-bg-secondary border-white/20 text-white shadow-lg scale-105 font-black"
                                      : "theme-bg-card theme-border-soft theme-text hover:theme-bg-soft"
                                }`}
                              >
                                {right}
                ...
                >
                  ...
                  <h2 className="text-xl md:text-2xl font-black mb-4 tracking-tighter font-afarin1">
                    {labels.completeTitle}
                  </h2>
                  <div className="flex gap-4 mt-8 relative z-10">
                    <div className="bg-white/5 border border-white/10 p-4 rounded-3xl min-w-[120px]">
                      <p className="text-xs font-black theme-primary uppercase tracking-widest mb-1">
                        {labels.xp}
                      </p>
                      <p className="text-2xl font-black text-white">+{Math.max(0, xp - lessonStartXp)}</p>
                    </div>
                    <div className="bg-white/5 border border-white/10 p-4 rounded-3xl min-w-[120px]">
                      <p className="text-xs font-black theme-primary uppercase tracking-widest mb-1">
                        {labels.accuracy}
                      ...
                      <p className="text-2xl font-black text-white">
                        {activeLesson ? Math.max(0, Math.round((activeLesson.exercises.length / (activeLesson.exercises.length + missedIndices.size)) * 100)) : 100}%
                      </p>
                    </div>
                  </div>
                </motion.div>
            ...
            <div className="mt-auto pt-8 shrink-0">
              {!feedback && !isLessonComplete ? (
                <motion.button
                  ...
                  disabled={
                    ...
                    currentEx.type === "fill-blank"
                      ...
                      : currentEx.type === "translation" ||
                          currentEx.type === "construct"
                        ? usedOptionIndices.length === 0
                        : currentEx.type === "match"
                          ? completedPairs.length <
                            (currentEx.pairs?.length || 0) * 2
                          : currentEx.type === "intro"
                            ? false
                            : false
                  }
                  onClick={
                    currentEx.type === "intro" ? nextExercise : checkAnswer
                  }
                ...
                <motion.div
                  initial={{ y: 100 }}
                  animate={{ y: 0 }}
                  className={`p-4 rounded-3xl border-t-[8px] flex items-center justify-between shadow-2xl ${
                    isLessonComplete
                ...
                >
                  <div className="flex flex-col gap-1">
                    <div className="flex items-center gap-2">
                      {feedback === "correct" ? (
                        <Trophy size={24} />
                      ) : (feedback === "wrong" || feedback === "wrong_first") ? (
                        <X size={24} />
                      ) : (
                        <CheckCircle size={24} />
                      )}
  ...
  return (
    <ErrorBoundary>
      <div
        className="min-h-screen theme-bg-page theme-text font-sans selection:bg-theme-primary/30 overflow-x-hidden"
        dir={isRTL ? "rtl" : "ltr"}
      >
        <ThemeInjected dialect={targetDialect} mode={themeMode} />
        <BackgroundBlobs dialect={targetDialect} />
        {/* Immersive background decoration */}
        <div className="fixed top-0 left-0 h-full w-full overflow-hidden pointer-events-none z-0 opacity-20">
            ...
            <AnimatePresence mode="wait">
              {activeTab === "home" ? (
                ...
                >
                  ...
                  <div className="relative overflow-hidden theme-bg-card backdrop-blur-lg md:backdrop-blur-xl rounded-[1.25rem] md:rounded-[1.75rem] p-4 md:p-4 border theme-border-soft shadow-2xl mb-6 md:mb-8 group">
                    <div className="absolute top-0 right-0 w-60 h-60 theme-bg-primary opacity-[0.03] blur-[80px] -translate-y-1/2 translate-x-1/2 pointer-events-none" />

                    <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-2">
                          <span className="px-2 py-0.5 bg-theme-primary/20 rounded-md text-[6px] md:text-[7px] theme-primary font-black uppercase tracking-[0.2em]">
                            {isSorani ? "ئەکادیمیای خانی" : "Akademiya Xanî"}
                          </span>
                        </div>
                        <h1 className="text-xl md:text-2xl font-black text-white tracking-tighter leading-none font-afarin1">
                          {isSorani ? "زاراوە شیرینەکانی" : "Zaravên Xweş ên"}{" "}
                          <span className="theme-primary italic block mt-1">
                            {isSorani ? "کوردستان" : "Kurdistanê"}
                          </span>
                        </h1>
                      </div>
                      ...
                      <div className="flex flex-wrap items-center gap-1.5">
                        {[
                          {
                            id: "Sorani",
                            label: isSorani ? "سۆرانی" : "Soranî",
                          },
                          {
                            id: "Kurmanji",
                            label: isSorani ? "کورمانجی" : "Kurmancî",
                          },
                          {
                            id: "Hawrami",
                            label: isSorani ? "هەورامی" : "Hewramî",
                          },
                          {
                            id: "Zazaki",
                            label: isSorani ? "زازاکی" : "Zazakî",
                          },
                          { id: "Luri", label: isSorani ? "لوڕی" : "Lurî" },
                        ].map((dialect) => {
                          const dTheme =
                            dialectThemes[dialect.id as TargetDialect];
                          const isActive = targetDialect === dialect.id;
                          return (
                            <button
                              key={dialect.id}
                              onClick={() => {
                                setTargetDialect(dialect.id as any);
                                playSound(clickAudio);
                              }}
                              className={`px-3 py-1.5 md:px-4 md:py-2 rounded-xl border text-[9px] md:text-[10px] font-black transition-all duration-300 relative overflow-hidden group/btn ${
                                isActive
                            ...
                            >
                              {isActive && (
                                <motion.div
                                  layoutId="dialect-active"
                                  className="absolute inset-0 bg-white/20 blur-xl translate-y-1/2"
                                />
                              )}
                  ...
                  <div className="flex flex-col items-center gap-8 pb-24">
                    {[1, 2, 3, 4].map((levelNum) => {
                      ...
                      const levelLabels: Record<
                        ...
                        { s: string; k: string }
                      > = {
                        1: { s: "بنەماکانی سەرەتا", k: "Bingehên Sereke" },
                        2: { s: "ژیانی ڕۆژانە", k: "Jiyana Rojane" },
                        3: { s: "پەیوەندی و گفتوگۆ", k: "Têkilî u Axaftin" },
                        4: { s: "ئەدەب و فەلسەفە", k: "Edeb u Felsefe" },
                      };

                      return (
                        <div key={levelNum} className="w-full space-y-4">
                          <div className="flex flex-col items-center gap-4">
                            {levelUnits.map((unit, unitIdx) => {
                              ...
                              const unitTheme =
                                themesArr[unit.id % themesArr.length];
                              const unitLearnedCount = unit.lessons.filter(
                                (l) =>
                                  learnedLessons.has(
                                    `${targetDialect}-${l.id}`,
                                  ),
                              ...
                              return (
                                ...
                                >
                                  ...
                                  >
                                    <div
                                      className={`overflow-hidden rounded-[1.25rem] md:rounded-[1.75rem] theme-bg-soft backdrop-blur-md border theme-border-soft transition-all duration-500`}
                                    >
                                      <div className="flex flex-col md:flex-row items-stretch min-h-[50px] relative z-10">
                                        <div
                                          className={`w-1.5 ${unitTheme.bg} transition-all`}
                                        />
                                        <div className="flex-1 p-3 md:p-5 flex flex-col justify-center">
                                          <div className="flex items-center gap-2 mb-1">
                                            <span
                                              className={`px-1.5 py-0.5 rounded-md ${unitTheme.bg} text-white font-black text-[6px] uppercase tracking-widest`}
                                            >
                                              {labels.unit} {unit.id}
                                            </span>
                                            <div className="w-1 h-1 rounded-full bg-white/10" />
                                            <span className="text-[6px] text-white/30 font-black uppercase tracking-widest">
                                              {unit.categories[0]}
                                            </span>
                                          </div>
                                          <h2 className="text-sm md:text-base font-black text-white tracking-tight font-afarin1">
                                            {unit.title}
                                          </h2>
                                        </div>
                                        <div className="p-3 md:px-5 flex flex-col items-center justify-center border-t md:border-t-0 md:border-s border-white/5 bg-black/20">
                                          ...
                                          <div className="w-14 h-0.5 bg-white/5 rounded-full overflow-hidden">
                                            <div
                                              className={`h-full ${unitTheme.bg}`}
                                              style={{
                                                width: `${(unitLearnedCount / unit.lessons.length) * 100}%`,
                                              }}
                                            />
                                          </div>
                                        </div>
                                  ...
                                  <div className="flex flex-col items-center gap-4 py-4">
                                    {unit.lessons.map((lesson, lessonIdx) => {
                                      const learned = learnedLessons.has(
                                        `${targetDialect}-${lesson.id}`,
                                      );
                                      const theme = unitTheme;

                                      // Determine unlock status based on global order
                                      const flatLessons = units.flatMap(
                                        (u) => u.lessons,
                                      );
                                      ...
                                      const lessonUnlocked =
                                        currentFlatIdx === 0 ||
                                        learnedLessons.has(
                                          `${targetDialect}-${flatLessons[currentFlatIdx - 1].id}`,
                                        );
                                      ...
                                      const offsetIdx =
                                        ...
                                        pathPattern.length;
                                      const offset =
                                        pathPattern[offsetIdx] * curveStrength;
                                        ...
                                        >
                                          ...
                                          >
                                            <motion.button
                                              ...
                                              className={`relative w-12 h-12 md:w-14 md:h-14 rounded-[1.25rem] md:rounded-[1.25rem] flex items-center justify-center transition-all group ${
                                                learned
                                                  ? `${theme.bg} ${theme.border} border-b-4 md:border-b-6 ${theme.shadow} shadow-lg`
                                                  : !lessonUnlocked
                                                    ? "bg-white/5 border-transparent opacity-20 cursor-not-allowed shadow-none"
                                                    : `bg-white/5 border-2 md:border-2 ${theme.activeBorder} ${theme.shadow} shadow-md`
                                              }`}
                                            >
                                              <div
                                                className={`relative z-10 ${learned ? "text-white" : lessonUnlocked ? theme.text : "text-white/40"}`}
                                              >
                                                {!lessonUnlocked ? (
                                                  <Moon
                                                    size={16}
                ...
                >
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                    <div className="flex items-center gap-4">
                      <div className="w-14 h-14 theme-bg-primary rounded-3xl flex items-center justify-center shadow-2xl rotate-3">
                        <Library size={32} className="text-white" />
                      </div>
                      <div>
                        <h2 className="text-2xl font-black text-white font-afarin1">
                          {labels.grammar}
                        </h2>
                        <p className="theme-primary font-bold uppercase tracking-widest text-[10px]">
                          {interfaceLang === "Sorani"
                            ? "فێربوونی شێوەزارە جیاوازەکان"
                            : "Fêrbûna devokên cihêreng"}
                        </p>
                      </div>
                    </div>
                    ...
                    {(schoolDialect || selectedSchoolCategory) &&
                      !selectedSchoolTopic && (
                        <button
                          onClick={() => {
                            playSound(clickAudio);
                  ...
                  !selectedSchoolTopic ? (
                    <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 md:gap-4">
                      {schoolGroupedCategories.categorized.map(({ item, count }, i) => {
                              return (
                                <motion.button
                                  ...
                                  className="group relative h-40 md:h-64 bg-white/[0.03] border border-white/10 rounded-[1.5rem] md:rounded-[1.25rem] p-4 md:p-4 flex flex-col justify-between overflow-hidden hover:border-theme-primary/40 transition-all shadow-2xl"
                                >
                                  ...
                                  <div className="relative z-10 flex justify-between items-start">
                                    <div className="w-10 h-10 md:w-14 md:h-14 rounded-xl md:rounded-[1.25rem] bg-white/5 flex items-center justify-center border border-white/10 group-hover:theme-bg-primary group-hover:border-transparent transition-all duration-500">
                                      {schoolGrouping === "book" ? (
                                        <Book
                                          size={24}
                                  ...
                                  <div className="relative z-10 text-left">
                                    ...
                                    <div className="flex items-center gap-1">
                                      ...
                                      <ChevronRight
                                        size={10}
                                        className="theme-primary opacity-0 group-hover:opacity-100 transition-all"
                                      />
                                    </div>
                                  </div>
                                </motion.button>
                      ...
                      {isAdminUser && schoolGrouping === "category" && (
                        <motion.button
                          ...
                          className="group relative h-40 md:h-64 border-2 border-dashed border-white/20 rounded-[1.5rem] md:rounded-[1.25rem] p-4 flex flex-col items-center justify-center gap-2 hover:border-theme-primary/30 transition-all"
                        >
                          <Plus
                            className="text-white/20 group-hover:theme-primary transition-colors"
                            size={32}
                          />
                      ...
                      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-10">
                        ...
                        <div className="flex items-center gap-2">
                          <div className="relative group">
                            <Search
                              className={`absolute left-4 top-1/2 -translate-y-1/2 transition-colors ${schoolSearchQuery ? "theme-primary" : "text-white/20"}`}
                              size={14}
                            />
                            <input
                              type="text"
                              ...
                              onClick={async () => {
                                ...
                                try {
                                  await setDoc(
                                    ...
                                    { topics: cleanObject(newTopicsList) },
                                    { merge: true },
                                  );
                                  playSound(clickAudio);
                                } catch (err) {
                            ...
                            <div className="grid grid-cols-1 gap-4">
                              {schoolFilteredTopics.map((topic, i) => (
                                  <motion.button
                                    ...
                                    className="bg-white/5 border border-white/10 hover:border-theme-primary/30 p-4 rounded-[1.5rem] flex items-center justify-between group transition-all"
                                  >
                                    <div className="flex items-center gap-4">
                                      <div
                                        className={`w-12 h-12 rounded-xl border flex items-center justify-center overflow-hidden transition-all duration-300 ${schoolTopicReadStatus.has(getTopicUniqueId(topic)) ? "bg-emerald-500/10 border-emerald-500/50 text-emerald-500 shadow-[0_0_15px_rgba(16,185,129,0.2)]" : "bg-white/5 border-white/10 text-white/20 group-hover:theme-primary group-hover:bg-theme-primary/10"}`}
                                      >
                                        {schoolTopicReadStatus.has(
                                          getTopicUniqueId(topic),
                                        ) ? (
                                          <CheckCircle size={24} />
                                        ) : topic.image ? (
                                          <img
                                      ...
                                      <div className="text-left">
                                        <span
                                          className={`block text-lg font-bold font-afarin1 transition-all ${schoolTopicReadStatus.has(getTopicUniqueId(topic)) ? "text-emerald-500/90" : "text-white/80 group-hover:text-white"}`}
                                        >
                                          {topic.title}
                                          {schoolTopicReadStatus.has(
                                            getTopicUniqueId(topic),
                        ...
                        <motion.button
                          ...
                          onClick={() => {
                            ...
                            if (title) {
                              ...
                              if (content) {
                                const dialect = (selectedSchoolCategory || "").trim();
                                if (!dialect) return;
                                const current =
                                  appConfig?.dialectSchoolContent?.[dialect] ||
                                  [];
                                saveGlobalConfig({
                                  dialectSchoolContent: {
                                    ...appConfig?.dialectSchoolContent,
                          ...
                          className="w-full py-6 border-2 border-dashed border-white/10 rounded-[1.25rem] flex items-center justify-center gap-4 text-white/40 hover:border-theme-primary/40 hover:text-white transition-all group mt-6"
                        >
                    ...
                    >
                      ...
                      <div className="bg-black border border-white/10 rounded-[1.5rem] overflow-hidden shadow-2xl relative">
                        {/* Decorative elements */}
                        ...
                        <div className="p-4 md:p-20 relative z-10">
                          ...
                          >
                            <div className="flex items-center gap-3">
                              <div className="px-5 py-2 bg-theme-primary/10 border border-theme-primary/20 rounded-full text-[9px] font-black uppercase tracking-[0.3em] theme-primary">
                                {selectedSchoolCategory || (interfaceLang === "Sorani" ? "گشتی" : "Giştî")}
                              </div>
                            ...
                            <div className="flex items-center gap-4 self-end md:self-auto flex-wrap justify-end">
                              <button
                                onClick={() =>
                                  toggleSchoolTopicRead(
                                    getTopicUniqueId(selectedSchoolTopic),
                                  )
                                }
                                ...
                                title={
                                  interfaceLang === "Sorani"
                                    ? "وەک خوێنراو دیاری بکە"
                                    : "Wek xwendî nîşan bike"
                                }
                              >
                                <CheckCircle
                                  size={16}
                              ...
                              <div className="flex items-center bg-white/5 border border-white/10 rounded-2xl overflow-hidden shadow-inner">
                                ...
                                <div className="px-3 border-x border-white/10 flex flex-col items-center justify-center min-w-[50px]">
                                  ...
                                  <span className="text-[7px] font-black uppercase tracking-tighter opacity-20 -mt-1">
                                    px
                                  </span>
                                </div>
                                <button
                              ...
                              <div className="flex items-center bg-white/5 border border-white/10 rounded-2xl p-1 gap-1">
                                {[
                                  ...
                                  <button
                                    key={tc.color}
                                    onClick={() => {
                                      setSchoolReadingColor(tc.color);
                                      playSound(clickAudio);
                                    }}
                          ...
                          >
                            <motion.h2
                              ...
                              className="text-lg md:text-xl font-black theme-text tracking-tighter leading-[1.1] mb-4 font-afarin1"
                            >
                          ...
                          >
                            <div
                              className="prose prose-invert max-w-none font-medium leading-[2.1] text-justify space-y-4 font-afarin2 p-4 md:p-4 rounded-[1.25rem] transition-colors duration-300"
                              style={{
                              ...
                              dangerouslySetInnerHTML={{
                                __html: (selectedSchoolTopic.content || "")
                                  ...
                                  .replace(
                                    /\[b\](.*?)\[\/b\]/gi,
                                    '<span class="font-black" style="color: inherit; opacity: 1; filter: brightness(1.3)">$1</span>',
                                  )
                                  .replace(
                                    /\[i\](.*?)\[\/i\]/gi,
                                    '<span class="italic" style="color: inherit; opacity: 0.8">$1</span>',
                                  )
                                  .replace(
                                    /\[center\](.*?)\[\/center\]/gi,
                          ...
                          >
                            ...
                            <div className="flex items-center justify-between gap-4">
                              {(() => {
                                ...
                                const prevTopic =
                                  currentIndex > 0
                                    ? dialectTopics[currentIndex - 1]
                                    : null;
                                const nextTopic =
                                  currentIndex < dialectTopics.length - 1
                                    ? dialectTopics[currentIndex + 1]
                                    : null;
                                ...
                                return (
                                  <>
                                    <button
                                      ...
                                      onClick={() => {
                                        playSound(clickAudio);
                                        setSelectedSchoolTopic(prevTopic);
                                      }}
                                    ...
                                    <button
                                      ...
                                      onClick={() => {
                                        playSound(clickAudio);
                                        setSelectedSchoolTopic(nextTopic);
                                      }}
                            ...
                            <div className="flex flex-col items-center gap-4">
                              <div className="w-14 h-1.5 theme-bg-primary/20 rounded-full" />
                            </div>
                          </motion.div>
                        </div>
                      </div>
                ...
                >
                  <div className="overflow-hidden theme-bg-card backdrop-blur-xl border theme-border-soft rounded-[1.25rem] md:rounded-[1.25rem] shadow-2xl relative">
                    {user ? (
                      <div className="flex flex-col">
                        ...
                        <div className="px-6 md:px-10 pb-10 flex flex-col items-center text-center -mt-14 md:-mt-20 relative z-10">
                          <div className="relative group">
                            ...
                            <div className="relative w-28 h-28 md:w-36 md:h-36 rounded-full border-8 border-[#0f172a] overflow-hidden mb-4 shadow-2xl p-1 bg-white/5">
                              {editPhotoURL ? (
                                ...
                                <div className="w-full h-full bg-gradient-to-br from-purple-500 to-indigo-600 flex items-center justify-center text-white font-black text-xl md:text-2xl rounded-full">
                                  {editDisplayName?.[0] ||
                          ...
                          <h2 className="text-xl md:text-2xl font-black theme-text mt-4 mb-1 font-afarin1 tracking-tight">
                            {user.displayName || user.email?.split("@")[0]}
                          </h2>
                          <p className="theme-text-muted text-[10px] md:text-xs font-bold tracking-widest uppercase opacity-40 mb-6">
                            {user.email}
                          </p>
                          ...
                          <div className="w-full max-w-lg space-y-4 mt-4">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                              <div className="text-left space-y-1.5">
                                <label className="text-[10px] font-black theme-primary uppercase tracking-widest ml-4 block font-afarin1">
                                  {labels.displayName}
                                </label>
                                <input
                                  type="text"
                          ...
                          <button
                            ...
                            className="mt-8 flex items-center gap-2 px-6 py-2 theme-bg-soft rounded-full text-[10px] font-black uppercase theme-text hover:theme-bg-soft-20 transition-all tracking-widest"
                          >
                      ...
                      <div className="flex flex-col items-center text-center mb-10">
                        ...
                        <h2 className="text-2xl font-black mb-6 theme-text opacity-90">
                          {labels.loginTitle}
                        </h2>
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-10">
                      <div className="theme-bg-card p-4 rounded-[1.5rem] border theme-border-soft text-center">
                        <p className="text-[10px] font-black theme-primary uppercase tracking-widest mb-2">
                          {labels.xp}
                        </p>
                        <p className="text-2xl font-black theme-text">{xp}</p>
                      </div>
                      <div className="theme-bg-card p-4 rounded-[1.5rem] border theme-border-soft text-center">
                        <p className="text-[10px] font-black text-orange-400 uppercase tracking-widest mb-2">
                          {labels.streak}
                        </p>
                        <p className="text-2xl font-black theme-text flex items-center justify-center gap-2">
                          {streak}{" "}
                          <Flame
                            size={24}
                    ...
                    <div>
                      <h3 className="text-sm font-black theme-primary uppercase tracking-widest mb-4 border-b theme-border-soft pb-2 font-afarin1">
                        {labels.interface}
                      </h3>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 pt-4">
                      <div className="flex items-center justify-between p-4 theme-bg-soft rounded-[1.5rem] border theme-border-soft w-full">
                        <div className="flex items-center gap-4">
                    ...
                    {/* Font Size Setting */}
                    <div className="col-span-full flex flex-col gap-4 p-4 bg-white/5 rounded-[1.5rem] border border-white/10 mt-6">
                    ...
                    <div className="col-span-full pt-12">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        {/* Mission */}
                        <div className="p-4 theme-bg-card border theme-border-soft rounded-[1.25rem] relative overflow-hidden group hover:theme-bg-soft transition-all duration-500">
                        ...
                        {/* Culture */}
                        <div className="p-4 theme-bg-card border theme-border-soft rounded-[1.25rem] relative overflow-hidden group hover:theme-bg-soft transition-all duration-500">
                          <div className="absolute -top-4 -right-4 p-4 opacity-5 group-hover:opacity-10 transition-opacity rotate-45">
                            <Languages size={120} />
                          </div>
                          <div className="w-12 h-12 theme-bg-primary rounded-2xl flex items-center justify-center text-white shadow-xl mb-6 group-hover:scale-110 transition-transform">
                            <Library size={24} />
                    ...
                    <div className="flex items-center justify-between p-4 bg-white/5 rounded-[1.5rem] border border-white/10">
                      <div className="flex items-center gap-4 text-red-100/40">
                        <Trash2 size={24} />
                        <span className="font-black text-sm uppercase tracking-widest opacity-20">
                      ...
                      <button
                        onClick={() =>
                          alert(
                            "Account deletion is disabled for safety in this version.",
                          )
                        }
                ...
                >
                  <div className="flex flex-col gap-2 mb-4">
                    <h2 className="text-2xl font-black text-white tracking-tighter flex items-center gap-3 font-afarin1">
                  ...
                  <div className="flex gap-2 overflow-x-auto pb-4 no-scrollbar -mx-2 px-2 sticky top-[60px] z-30 theme-glass py-2">
                    {[
                      {
                        id: "general",
                        label: interfaceLang === "Sorani" ? "گشتی" : "Giştî",
                        icon: Settings,
                      },
                      {
                        id: "labels",
                        label:
                          interfaceLang === "Sorani" ? "نیشانەکان" : "Nîşan",
                        icon: Palette,
                      },
                      {
                        id: "tabs",
                        label: interfaceLang === "Sorani" ? "تابەکان" : "Tab",
                        icon: Layers,
                      },
                      {
                        id: "school",
                        label: interfaceLang === "Sorani" ? "وانەکان" : "Ders",
                        icon: GraduationCap,
                      },
                      {
                        id: "dictionary",
                        label:
                          interfaceLang === "Sorani" ? "فەرهەنگ" : "Ferheng",
                        icon: Book,
                      },
                      {
                        id: "lessons",
                  ...
                  <div className="space-y-10">
                    {adminSubTab === "general" && (
                      <motion.div
                        ...
                        className="space-y-4"
                      >
                        <div className="theme-bg-card p-4 rounded-[1.25rem] border theme-border-soft space-y-4">
                          <div className="flex items-center justify-between">
                            <div>
                              <h4 className="font-black text-white uppercase tracking-widest text-sm mb-1">
                                {interfaceLang === "Sorani"
                                  ? "باری گەشەپێدەر"
                                  : "Moda Geşepêder"}
                              </h4>
                              <p className="text-[10px] theme-text-muted font-bold">
                                Show admin highlights and debug info
                              </p>
                              {user && (
                                <p className="text-[10px] text-white/40 mt-1 select-all cursor-pointer">
                                  UID: {user.uid} (بۆ کۆپی کردن کلیک بکە)
                                </p>
                              )}
                            </div>
                            <button
                          ...
                          <div className="pt-6 border-t theme-border-soft">
                            <h4 className="font-black text-red-500 uppercase tracking-widest text-sm mb-4">
                              {interfaceLang === "Sorani"
                                ? "کردارە ترسناکەکان"
                                : "Karên Tirsnak"}
                            </h4>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 max-h-[60vh] overflow-y-auto pr-2 custom-scrollbar">
                          {Object.entries(labels)
                            ...
                            .map(([key, value]) => (
                              <div
                                key={key}
                                className="bg-white/5 p-4 rounded-2xl border theme-border-soft group hover:border-white/20 transition-all"
                              >
                                <label className="block text-[8px] font-black theme-primary uppercase tracking-widest mb-1 opacity-40 font-afarin1">
                                  {key}
                                </label>
                                <SyncInput
                                  value={value as string}
                                  onSync={(val: string) =>
                                    updateGlobalLabel(key, val)
                                  }
                          ...
                          {(appConfig?.extraTabs || []).map(
                            (t: any, idx: number) => (
                              ...
                              >
                                <div className="flex items-center justify-between mb-4">
                                  <div className="flex items-center gap-4">
                                    <span className="w-8 h-8 rounded-full bg-white/5 flex items-center justify-center font-black text-xs text-white/40">
                                      {idx + 1}
                                    </span>
                                    <SyncInput
                                      value={t.label}
                                    ...
                                    {deletingTabId === t.id ? (
                                      ...
                                      <>
                                        <button
                                          ...
                                          className="p-2 bg-white/5 rounded-xl text-white/40 disabled:opacity-0"
                                        >
                                          <ArrowUp size={16} />
                                        </button>
                                        <button
  ...
  >
    ...
    {!selectedSchoolCategory ? (
           ...
           <div className="space-y-4">
              {(() => {
                 ...
                 return topics.map((topic, _idx) => {
                    ...
                    return (
                      <AdminSchoolTopicEditor
                        key={`topic-editor-${_idx}-${topic.title}`}
                        topic={topic}
                    ...
                    {adminSubTab === "dictionary" && (
                      ...
                      >
                        <div className="flex flex-col sm:flex-row gap-4 mb-6">
                          <div className="relative flex-1">
                            ...
                            <input
                              ...
                              placeholder="Search dictionary (Sorani/Kurmanji/English)..."
                              className="w-full bg-white/5 border theme-border-soft rounded-2xl pl-12 pr-6 py-4 text-sm text-white font-bold outline-none focus:theme-border-primary transition-all"
                            />
                          </div>
                          <button
                            onClick={() => setShowAddWordModal(true)}
                            className="px-8 py-4 theme-bg-primary text-white rounded-2xl font-black uppercase tracking-widest shadow-lg flex items-center gap-2 hover:scale-105 transition-all"
                          >
                            <Plus size={20} />
                            <span>
                              {interfaceLang === "Sorani"
                        ...
                        {showAddWordModal && (
                          <div className="bg-white/5 border border-amber-400/30 rounded-[1.25rem] p-4 space-y-4 mb-8">
                            <div className="flex justify-between items-center mb-4">
                              <h4 className="text-lg font-black text-amber-400 font-afarin1">
                                زیادکردنی وشە / Zêdekirina Peyvê
                              </h4>
                              <button
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                              <input
                                placeholder="Sorani..."
                                value={newWord.sorani}
                                onChange={(e) =>
                                  setNewWord({
                                    ...newWord,
                                    sorani: e.target.value,
                                  })
                                }
                                className="bg-black/40 border theme-border-soft rounded-xl px-4 py-3 text-sm text-white"
                              />
                              <input
                                placeholder="Kurmanji..."
                                value={newWord.kurmanji}
                                onChange={(e) =>
                                  setNewWord({
                                    ...newWord,
                                    kurmanji: e.target.value,
                                  })
                                }
                                className="bg-black/40 border theme-border-soft rounded-xl px-4 py-3 text-sm text-white"
                              />
                            ...
                            <button
                              onClick={async () => {
                                if (!newWord.sorani) return;
                                await saveDictionaryWord(
                                  { id: "custom_" + Date.now(), ...newWord },
                                  {},
                                );
                                setNewWord({
                                  sorani: "",
                              ...
                              className="w-full py-4 theme-bg-primary text-white rounded-2xl font-black uppercase tracking-widest mt-4"
                            >
                              تەواو / Zêde Bike
                            </button>
                          </div>
                            ...
                            .map((word, idx) => {
                              ...
                              return (
                                ...
                                >
                                  <div className="flex items-center justify-between mb-4">
                                    <span
                                      className={`text-[8px] font-black px-2 py-0.5 rounded-md uppercase tracking-widest ${custom ? "bg-amber-400 text-black" : "bg-white/10 text-white/40"}`}
                                    >
                                      {custom ? "Custom" : "Core"}
                                    </span>
                                    <span className="text-[10px] theme-text-muted font-bold opacity-30">
                                      {wordId}
                                    </span>
                                  </div>
                                  ...
                                  <div className="space-y-4">
                                    <div className="space-y-1">
                                      <label className="text-[8px] font-black uppercase text-amber-400/60 ml-1">
                                        Sorani
                                      </label>
                                      <SyncInput
                                        value={currentWord.sorani}
                                        onSync={(val: string) =>
                                          saveDictionaryWord(word, {
                                            sorani: val,
                                          })
                                        }
                                        className="w-full bg-black/40 border theme-border-soft rounded-xl px-4 py-2 text-xs text-white outline-none focus:theme-border-primary font-afarin2"
                                      />
                                    </div>
                                    <div className="space-y-1">
                                      <label className="text-[8px] font-black uppercase text-amber-400/60 ml-1">
                        ...
                        {dictSearch === "" && academicDict.length > 100 && (
                          <p className="text-center text-[10px] theme-text-muted font-bold opacity-40 uppercase tracking-widest">
                            Type to search through {academicDict.length}{" "}
                            words...
                          </p>
                        )}
                      </motion.div>
                    ...
                    {adminSubTab === "lessons" && (
                      ...
                      >
                        <div className="flex flex-col gap-4">
                          <h3 className="text-xl font-black theme-text font-afarin1">
                            دەستکاری پرسیارەکانی وانە / Edîtorê Pirsan
                          </h3>
                          <div className="flex gap-2 overflow-x-auto pb-2 no-scrollbar">
                            {[
                              ...
                              <button
                                ...
                                onClick={() => {
                                  playSound(clickAudio);
                                  setActiveAdminLessonDialect(
                                    d as TargetDialect,
                                  );
                                  setAdminSelectedLessonId(null);
                                }}
                                className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest border transition-all ${
                                  activeAdminLessonDialect === d
                                    ? "theme-bg-primary text-white border-white/20 shadow-lg"
                                    : "bg-white/5 theme-text-muted border-white/5 hover:bg-white/10"
                                }`}
                              >
                                {d}
                              </button>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                          {adminUnits
                            ...
                            .map((lesson) => (
                              ...
                              >
                                <div>
                                  <p className="font-bold theme-text uppercase tracking-widest text-xs opacity-50 mb-1">
                                    LESSON ID
                                  </p>
                                  <p className="font-black theme-text truncate max-w-[150px]">
                                    {lesson.id}
                                  </p>
                                </div>
                                <button
                                  onClick={() => {
                                    setAdminSelectedLessonId(
                                      lesson.id.toString(),
                                    );
                                    setEditingExercise(null);
                                  }}
                                  className="w-10 h-10 rounded-full theme-bg-primary text-white flex items-center justify-center active:scale-95 transition-all"
                                >
                                  <Edit3 size={16} />
                                </button>
                              </div>
                        ...
                        {adminSelectedLessonId && (
                          <div className="mt-8 p-4 theme-bg-card rounded-3xl border theme-border-soft space-y-4">
                            {(() => {
                              const lessonDetails = adminUnits
                                .flatMap((u) => u.lessons)
                                .find(
                                  (l) =>
                                    l.id.toString() === adminSelectedLessonId,
                                );
                              if (!lessonDetails) return null;
                              return (
                                <>
                                  <h4 className="text-lg font-black theme-text mb-4">
                                    Exercises in {adminSelectedLessonId}
                                  </h4>
                                    ...
                                    {lessonDetails.exercises.map((ex, i) => (
                                      ...
                                      >
                                        <div className="flex-1 pr-4">
                                          <p className="font-black theme-text text-sm mb-1">
                                            {typeof ex.question === "string"
                                              ? ex.question
                                              : `${ex.question?.Sorani || ""} / ${ex.question?.Kurmanji || ""}`}
                                          </p>
                                          <p className="theme-text-muted text-xs truncate max-w-sm">
                                            {JSON.stringify(ex.answer)}
                                          </p>
                                          <p className="theme-text-muted text-[10px] uppercase opacity-50 mt-1">
                                            {ex.type}
                                          </p>
                                        </div>
                                        <button
                                          onClick={() => {
                                            ...
                                            setActiveLessonId(lessonDetails.id);
                                            setExerciseIndex(i);
                                            setShowEditModal(true);
                                          }}
                                          className="px-4 py-2 theme-bg-primary text-white text-xs font-black uppercase tracking-widest rounded-xl hover:opacity-80 transition-opacity whitespace-nowrap"
                                        >
                                          دەستکاری
                                        </button>
                                      </div>
                ...
                <motion.div
                  ...
                  animate={{ opacity: 1 }}
                  exit={{ opacity: 0 }}
                  className="absolute inset-0 z-20 bg-black pb-[90px] lg:pb-0 lg:rounded-tl-2xl overflow-hidden"
                >
                  ...
                  <iframe
                    src="https://ferga-transtale.vercel.app/"
                ...
                >
                  <div className="flex items-center gap-4 mb-6 w-full max-w-xl">
                    <div className="w-14 h-14 theme-bg-soft rounded-[1.25rem] flex items-center justify-center shadow-2xl border theme-border-soft">
                    ...
                    <div>
                      ...
                      <p className="theme-primary font-bold uppercase tracking-widest text-[10px]">
                        {interfaceLang === "Sorani"
                          ? "وشەکانی خۆت بسپێرە بە بیرەوەری"
                          : "Peyvên xwe bispêre bîrgehê"}
                      </p>
                    </div>
                  </div>
                  {!flashcardSessionActive ? (
                    <div className="w-full max-w-xl bg-white/5 border border-white/10 rounded-[1.25rem] p-4 text-center backdrop-blur-2xl shadow-2xl">
                      <h2 className="text-2xl font-black text-white mb-4 font-afarin1">
                        {interfaceLang === "Sorani"
                          ? "ئامادەیت بۆ پێداچوونەوە؟"
                          : "Ji bo dubarekirinê amade yî?"}
                      </h2>
                      <p className="text-sm font-bold theme-text-muted mb-6">
                          {interfaceLang === "Sorani" ? `تۆ ${activeFlashcards.length} وشەت هەیە بۆ پێداچوونەوە ئەمڕۆ` : `Tu xwedî ${activeFlashcards.length} peyvan î ji bo dubarekirina îro`}
                      </p>
                      <motion.button
                        onClick={() => setFlashcardSessionActive(true)}
                        disabled={activeFlashcards.length === 0}
                        className={`w-full py-4 rounded-2xl font-black text-white shadow-xl ${activeFlashcards.length > 0 ? "theme-bg-primary hover:opacity-90" : "bg-white/10 cursor-not-allowed opacity-50"}`}
                      >
                        {interfaceLang === "Sorani"
                          ? "دەستپێبکە"
                          : "Dest pê bike"}
                    ...
                    <div className="w-full max-w-2xl px-4 flex flex-col items-center">
                      <div className="w-full flex justify-between items-center mb-10 px-2">
                        <span className="text-[10px] font-black theme-primary uppercase tracking-[0.3em] font-sans">
                          {currentFlashcardIdx + 1} / {activeFlashcards.length}
                        </span>
                        <div className="flex-1 mx-8 h-2 theme-bg-soft rounded-full overflow-hidden border theme-border-soft">
                          <motion.div
                            initial={{ width: 0 }}
                            animate={{
                              width: `${((currentFlashcardIdx + 1) / activeFlashcards.length) * 100}%`,
                            }}
                      ...
                      <div
                        className="w-full h-[450px] relative mb-6"
                        ...
                        <AnimatePresence mode="wait">
                          ...
                          >
                            ...
                            >
                              {/* Front */}
                              <div
                                className="absolute inset-0 w-full h-full theme-bg-card theme-border-soft border-2 rounded-[1.5rem] shadow-2xl flex flex-col items-center justify-center p-4 backface-hidden"
                                style={{
                                  backfaceVisibility: "hidden",
                                  WebkitBackfaceVisibility: "hidden",
                                }}
                              >
                                <div className="mb-8 theme-bg-soft w-14 h-14 rounded-2xl flex items-center justify-center border theme-border-soft">
                                  <Languages
                                    size={28}
                                    className="theme-primary opacity-50"
                                  />
                                </div>
                                <h3 className="text-2xl md:text-6xl font-black theme-text tracking-tighter mb-4 text-center leading-tight font-afarin1">
                                ...
                                <div className="absolute bottom-12 opacity-20 font-black text-[9px] uppercase tracking-[0.4em] flex items-center gap-2">
                                  ...
                                  {interfaceLang === "Sorani"
                                    ? "دایبگرە بۆ وەرگێڕان"
                                    : "Ji bo wateyê bitikîne"}
                                </div>
                              </div>
                              {/* Back */}
                              <div
                                className="absolute inset-0 w-full h-full theme-bg-soft border-2 theme-border-soft rounded-[1.5rem] shadow-2xl flex flex-col items-center justify-center p-4 backface-hidden"
                                style={{
                                  backfaceVisibility: "hidden",
                                  WebkitBackfaceVisibility: "hidden",
                              ...
                              >
                                ...
                                <h3 className="text-xl md:text-2xl font-black text-white tracking-tight mb-8 text-center leading-tight font-afarin1">
                                  {activeFlashcards[currentFlashcardIdx]?.back}
                                </h3>
                                <div className="w-14 h-1.5 theme-bg-primary rounded-full mb-8 mx-auto" />
                                <p className="text-sm md:text-lg font-medium theme-text-muted italic leading-relaxed max-w-sm text-center px-4 font-afarin2">
                                  "{activeFlashcards[currentFlashcardIdx]?.example}"
                                </p>
                              </div>
                            </motion.div>
                          </motion.div>
                      ...
                      <div className="flex gap-4 md:gap-4 w-full max-w-lg mb-6">
                        <motion.button
                          ...
                          onClick={(e) => {
                            ...
                            if (next >= activeFlashcards.length)
                              setFlashcardSessionActive(false);
                            else {
                              setCurrentFlashcardIdx(next);
                              setIsFlipped(false);
                            }
                          }}
                          className="flex-1 bg-white/5 border border-white/10 text-white/50 py-5 rounded-[1.5rem] font-bold text-xs uppercase tracking-widest hover:bg-white/10 hover:text-white transition-all shadow-xl"
                        >
                        ...
                        <motion.button
                          ...
                          className="flex-[2] theme-bg-primary text-white py-5 rounded-[1.5rem] font-black text-xs uppercase tracking-widest shadow-2xl theme-shadow-glow"
                        >
                          {interfaceLang === "Sorani"
                            ? "دەیزانم"
                            : "Ez dizanim"}
                      ...
                      <button
                        onClick={() => setFlashcardSessionActive(false)}
                        className="text-white/20 hover:text-white/40 font-black uppercase text-[10px] tracking-[0.3em] transition-colors flex items-center gap-3 group"
                      >
                        <ChevronLeft
                          size={16}
                          className="group-hover:-translate-x-1 transition-transform"
                ...
                >
                  <div className="flex items-center gap-4 mb-6">
                    <div className="w-14 h-14 theme-bg-primary rounded-[1.25rem] flex items-center justify-center shadow-2xl">
                      <Trophy size={32} className="text-white" />
                    </div>
                    <div>
                      <h2 className="text-2xl font-black text-white tracking-tighter font-afarin1">
                        {labels.leaderboard}
                      </h2>
                      <p className="theme-primary font-bold uppercase tracking-widest text-[9px] font-sans opacity-70">
                        {interfaceLang === "Sorani"
                          ? "ڕیزبەندی و دەستکەوتەکان"
                          : "Rêzbendî û destkeftî"}
                      </p>
                    </div>
                  </div>
                        ...
                        {leaderboard.length > 0 ? (
                          leaderboard.slice(0, 30).map((u, i) => (
                            <div
                              ...
                              className={`flex items-center justify-between p-5 rounded-[1.5rem] border transition-all relative overflow-hidden group cursor-pointer hover:scale-[1.02] active:scale-[0.98] ${u.id === user?.uid ? "theme-bg-soft border-yellow-400/30 shadow-2xl" : "theme-bg-card theme-border-soft"}`}
                            >
                              {u.id === user?.uid && (
                                <div className="absolute left-0 top-0 bottom-0 w-1 theme-bg-primary" />
                              )}
                              <div className="flex items-center gap-5">
                                <div
                                  className={`w-10 h-10 rounded-full flex items-center justify-center font-black text-sm italic ${
                                    i === 0
                                      ? "bg-yellow-400 text-yellow-900 shadow-[0_0_20px_rgba(250,204,21,0.4)]"
                                      : i === 1
                                ...
                                <div className="w-12 h-12 rounded-2xl overflow-hidden border-2 theme-border-soft shadow-inner">
                                  <img
                                    src={
                                      u.photoURL ||
                                      `https://api.dicebear.com/7.x/avataaars/svg?seed=${u.displayName}`
                                    }
                                    alt="avatar"
                                    className="w-full h-full object-cover"
                                    referrerPolicy="no-referrer"
                                ...
                                <div>
                                  <p
                                    className={`text-base font-black font-afarin1 ${u.id === user?.uid ? "text-white" : "theme-text"}`}
                                  >
                                    {u.displayName || "Bikarhêner"}
                                  </p>
                                  <div className="flex items-center gap-2 mt-0.5">
                                    <Flame
                                      ...
                                      className="theme-primary"
                                    />
                                    <p className="text-[10px] theme-text opacity-40 font-bold uppercase tracking-widest">
                                      {u.xp} XP
                                    </p>
                                  </div>
                                </div>
                              ...
                              <div className="flex flex-col items-end">
                                <div className="w-12 h-12 rounded-2xl theme-bg-soft border theme-border-soft flex items-center justify-center group-hover:theme-bg-primary transition-colors">
                                  <Star
                                    size={18}
                                    className="theme-primary group-hover:text-white transition-colors"
                                  />
                                </div>
                              </div>
                            </div>
                          ...
                          <div className="py-24 text-center">
                            <div className="w-14 h-14 theme-bg-soft rounded-full flex items-center justify-center mx-auto mb-6 border theme-border-soft">
                              <Loader2
                                size={32}
                                className="theme-primary animate-spin"
                              />
                            </div>
                            <p className="opacity-20 font-black uppercase text-xs tracking-widest font-afarin1">
                              {labels.checking}...
                            </p>
                          </div>
                        )}
                      ...
                      >
                        {ACHIEVEMENTS.map((ach) => {
                          ...
                          return (
                            <div
                              key={ach.id}
                              className={`p-4 rounded-[1.25rem] border transition-all flex flex-col items-center text-center group ${isEarned ? "theme-bg-soft theme-border-soft-50 shadow-xl" : "theme-bg-card opacity-20 saturate-0 grayscale"}`}
                            >
                              <div
                                className={`w-14 h-14 rounded-[1.25rem] mb-6 flex items-center justify-center text-white shadow-lg ${isEarned ? ach.color : "bg-gray-700/50"}`}
                              >
                                <Star
                                  size={28}
                              ...
                              <h4 className="text-sm font-black theme-text mb-2 uppercase tracking-tighter font-afarin1">
                                {isRTL ? ach.title.Sorani : ach.title.Kurmanji}
                              </h4>
                              <p className="text-[10px] theme-text opacity-50 font-medium leading-relaxed font-afarin2">
                                {isRTL
                                  ? ach.description.Sorani
                                  : ach.description.Kurmanji}
                              </p>
                            </div>
                          );
                        })}
              ...
              ) : (appConfig?.extraTabs || []).some(
                  (t: any) => t.id === activeTab,
                ) ? (
                <motion.div
                  initial={{ opacity: 0, x: 20 }}
                  animate={{ opacity: 1, x: 0 }}
                  className="p-4 theme-bg-card border theme-border-soft rounded-[1.25rem] min-h-[60vh] backdrop-blur-3xl"
                >
                  ...
                  <div
                    ...
                    dangerouslySetInnerHTML={{
                      __html: (
                        appConfig.extraTabs.find((t: any) => t.id === activeTab)
                          ?.content || ""
                      ).replace(/\n/g, "<br/>"),
                    }}
                  />
          ...
          {selectedUserForDetail && (
            ...
            >
              ...
              <motion.div
                ...
                className="relative w-full max-w-lg theme-bg-card rounded-[1.25rem] border theme-border-soft overflow-hidden shadow-2xl"
              >
                {/* Banner */}
                <div className="h-40 relative group overflow-hidden bg-slate-900">
                  {selectedUserForDetail.bannerUrl ? (
                    <img
                      src={selectedUserForDetail.bannerUrl}
                      alt="Banner"
                      className="w-full h-full object-cover"
                    />
                  ) : (
                  ...
                  <button
                    onClick={() => setSelectedUserForDetail(null)}
                    className="absolute top-4 right-6 w-10 h-10 bg-black/50 backdrop-blur-md rounded-full flex items-center justify-center text-white hover:bg-black/70 transition-colors z-20"
                  >
                    <X size={20} />
                  </button>
                </div>
                ...
                <div className="px-8 pb-10 flex flex-col items-center text-center -mt-16 relative z-10">
                  <div className="w-32 h-32 rounded-full border-8 border-[#0f172a] overflow-hidden mb-4 shadow-2xl p-1 bg-white/5 relative">
                    <img
                      src={
                        ...
                        `https://api.dicebear.com/7.x/avataaars/svg?seed=${selectedUserForDetail.displayName || "u"}`
                      }
                      alt="Avatar"
                      className="w-full h-full object-cover rounded-full"
                      referrerPolicy="no-referrer"
                  ...
                  <h3 className="text-xl md:text-2xl font-black text-white mt-4 mb-1 font-afarin1 tracking-tight">
                  ...
                  <div className="flex items-center gap-3 mb-6">
                    <span className="px-3 py-1 theme-bg-primary rounded-full text-[10px] font-black text-white uppercase tracking-widest">
                      {labels.level}{" "}
                      {Math.floor((selectedUserForDetail.xp || 0) / 1000) + 1}
                    </span>
                    <div className="w-1.5 h-1.5 rounded-full bg-white/10" />
                    <span className="text-[10px] theme-text opacity-40 font-black uppercase tracking-widest">
                      Ranked #
                      {leaderboard.findIndex(
                        (u) => u.id === selectedUserForDetail.id,
                      ) + 1}
                    </span>
                  </div>

                  {selectedUserForDetail.bio && (
                    <div className="p-4 bg-white/5 rounded-3xl border border-white/5 mb-8 w-full">
                  ...
                  <div className="grid grid-cols-2 gap-4 w-full">
                    <div className="p-4 theme-bg-soft rounded-3xl border theme-border-soft flex flex-col items-center justify-center">
                      <Flame size={24} className="theme-primary mb-2" />
                      <span className="text-2xl font-black theme-text leading-none mb-1">
                        {selectedUserForDetail.xp || 0}
                      </span>
                      <span className="text-[9px] font-black uppercase theme-primary tracking-widest opacity-60">
                        Total XP
                      </span>
                    </div>
                    <div className="p-4 theme-bg-soft rounded-3xl border theme-border-soft flex flex-col items-center justify-center">
                      ...
                      <span className="text-2xl font-black theme-text leading-none mb-1">
                        {Array.isArray(selectedUserForDetail.learnedLessons)
                          ? selectedUserForDetail.learnedLessons.length
                          : 0}
                      </span>
                      <span className="text-[9px] font-black uppercase theme-primary tracking-widest opacity-60">
      ...
      `}</style>
        ...
        <AnimatePresence>
          {showCropper && imageToCrop && (
            <motion.div
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="fixed inset-0 z-[100] bg-black/90 backdrop-blur-xl flex flex-col items-center justify-center p-4 md:p-4"
            >
              <div className="relative w-full max-w-2xl h-[60vh] md:h-[70vh] bg-black rounded-[1.5rem] overflow-hidden shadow-2xl border border-white/10">
                <Cropper
                  image={imageToCrop}
                  crop={crop}
                  zoom={zoom}
        ...
        {/* Toast Notification */}
        <AnimatePresence>
          {showEditModal && editingExercise && activeLesson && (
            <div
            ...
            >
              <motion.div
                initial={{ opacity: 0, scale: 0.95 }}
                animate={{ opacity: 1, scale: 1 }}
                className="theme-bg-card border theme-border-soft w-full max-w-2xl rounded-[1.25rem] p-4 space-y-4 shadow-2xl relative overflow-hidden max-h-[90vh] overflow-y-auto custom-scrollbar"
              >
                <div className="flex items-center justify-between mb-2">
                  <div>
                    <h3 className="text-xl font-black theme-text font-afarin1">
                      دەستکاری تێست / Sererastkirina Testê
                    </h3>
                    <p className="text-[10px] theme-text-muted font-bold uppercase tracking-widest opacity-50">
                ...
                <div className="space-y-4">
                  {/* Question Editing */}
                  <div className="bg-white/5 border theme-border-soft rounded-3xl p-4 space-y-5">
                    <div className="flex items-center justify-between mb-2">
                      <h4 className="text-sm font-black theme-primary uppercase tracking-widest font-afarin1">
                        دەقی پرسیار / Pirsa Testê
                      </h4>
                      <div className="flex bg-black/40 rounded-xl p-1">
                        <div className="px-3 py-1 text-[10px] font-black uppercase tracking-widest text-white/50 border-r border-white/10">
                          Bilingual
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-2">
                        <label className="flex items-center gap-2 text-[10px] font-black text-rose-400 uppercase tracking-widest opacity-80">
                        ...
                        <textarea
                          ...
                          dir="rtl"
                          value={
                            typeof editingExercise.question === "object"
                              ? (editingExercise.question as any).Sorani
                          ...
                          onChange={(e) =>
                            setEditingExercise({
                              ...
                              question: {
                                ...
                                Kurmanji:
                                  typeof editingExercise.question === "object"
                                    ? (editingExercise.question as any).Kurmanji
                                    : editingExercise.question,
                              },
                            })
                          }
                          className="w-full bg-black/40 border theme-border-soft rounded-2xl px-5 py-4 text-white font-bold outline-none focus:border-rose-500 transition-colors font-afarin1 text-sm resize-none text-right"
                          placeholder="لێرە پرسیارەکە بنووسە..."
                        />
                      </div>
                      <div className="space-y-2">
                        <label className="flex items-center gap-2 text-[10px] font-black text-amber-400 uppercase tracking-widest opacity-80">
                        ...
                        <textarea
                          ...
                          className="w-full bg-black/40 border theme-border-soft rounded-2xl px-5 py-4 text-white font-bold outline-none focus:border-amber-500 transition-colors font-afarin1 text-sm resize-none text-left"
                          placeholder="Pirsa xwe li vir binivîse..."
                        />
                      </div>
                    </div>
                  </div>
                  ...
                  {/* Correct Answer */}
                  {editingExercise.type === "match" && editingExercise.pairs ? (
                    <div>
                      <label className="block text-[10px] font-black theme-primary uppercase tracking-widest mb-2 opacity-50 font-afarin1">
                        جووتەکان / Cotkirin (Pairs)
                      </label>
                      <div className="grid grid-cols-1 gap-3">
                        {editingExercise.pairs.map((pair, i) => (
                          <div key={`key-prefix-7-${i}`} className="flex gap-2">
                            <input
                              ...
                              value={pair.left}
                              onChange={(e) => {
                                const newPairs = [...editingExercise.pairs!];
                                newPairs[i] = { ...newPairs[i], left: e.target.value };
                                setEditingExercise({
                                  ...editingExercise,
                                  pairs: newPairs,
                                  options: newPairs.map(p => p.left),
                                  answer: newPairs.map(p => p.right),
                                });
                              }}
                              className="flex-1 bg-black/20 border theme-border-soft rounded-xl px-4 py-3 text-xs text-white/80 font-bold outline-none focus:theme-border-primary"
                            />
                            <input
                              type="text"
                              value={pair.right}
                              ...
                              className="flex-1 bg-black/20 border theme-border-soft rounded-xl px-4 py-3 text-xs text-white/80 font-bold outline-none focus:theme-border-primary text-right"
                              dir="rtl"
                            />
                          </div>
                        ))}
                    ...
                    <>
                      <div>
                        <label className="block text-[10px] font-black theme-primary uppercase tracking-widest mb-2 opacity-50 font-afarin1">
                          وەڵامی ڕاست / Bersiva Rast
                        </label>
                        <input
                          type="text"
                          value={typeof editingExercise.answer === "string" ? editingExercise.answer : JSON.stringify(editingExercise.answer)}
                          onChange={(e) =>
                            setEditingExercise({
                              ...editingExercise,
                          ...
                          className="w-full bg-black/40 border theme-border-soft rounded-2xl px-6 py-4 text-white font-bold outline-none focus:theme-border-primary font-afarin2"
                        />
                      </div>
                      ...
                      <div>
                        <label className="block text-[10px] font-black theme-primary uppercase tracking-widest mb-2 opacity-50 font-afarin1 flex items-center justify-between">
                          <span>
                            لینک (Direct link / .jpg / .png) لینکی وێنە / Lînkê Wêne
                          </span>
                        </label>
                        <input
                          type="text"
                          ...
                          <div className="grid grid-cols-1 gap-3">
                            {editingExercise.options.map((opt, i) => (
                              <input
                                ...
                                value={opt}
                                onChange={(e) => {
                                  const newOpts = [...editingExercise.options!];
                                ...
                                className="w-full bg-black/20 border theme-border-soft rounded-xl px-4 py-3 text-xs text-white/80 font-bold outline-none focus:theme-border-primary"
                              />
                            ))}
                          </div>
                        </div>
                  ...
                  {/* Underlying Dictionary Word (Legacy Sync) */}
                  {editingExercise.originalItem && (
                    <div className="p-4 bg-white/5 border theme-border-soft rounded-2xl space-y-4">
                      <span className="text-[9px] font-black theme-primary uppercase tracking-widest">
                        فەرهەنگی بنەڕەتی / Dictionary Base
                      </span>
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <label className="block text-[8px] font-black theme-text-muted uppercase mb-1 opacity-50">
                            Sorani
                          </label>
                          <input
                            type="text"
                            defaultValue={editingExercise.originalItem.sorani}
                            onBlur={(e) =>
                              saveDictionaryWord(editingExercise.originalItem, {
                                sorani: e.target.value,
                              })
                            }
                            className="w-full bg-black/20 border theme-border-soft rounded-lg px-3 py-2 text-[10px] text-white"
                          />
                        </div>
                        <div>
                          <label className="block text-[8px] font-black theme-text-muted uppercase mb-1 opacity-50">
                          ...
                          <input
                            type="text"
                            defaultValue={editingExercise.originalItem.kurmanji}
                            onBlur={(e) =>
                              saveDictionaryWord(editingExercise.originalItem, {
                                kurmanji: e.target.value,
                              })
                ...
                <div className="flex gap-4">
                  ...
                  <button
                    onClick={() => {
                      const updatedExercises = [...activeLesson.exercises];
                      updatedExercises[exerciseIndex] = { ...editingExercise };
                      saveLessonOverride(
                        activeLesson.id.toString(),
                        updatedExercises,
                      );
                      setShowEditModal(false);
                      setActiveLessonId(null);
                    }}
                    className="flex-[2] py-4 theme-bg-primary text-white rounded-2xl font-black uppercase tracking-widest shadow-xl active:scale-95 transition-all"
                  >
                    پاشەکەوت / Qeyd Bike
                  </button>
                </div>
          ...
          {toast && (
            ...
            >
              ...
              <span className="text-sm font-black text-white font-afarin1">
                {toast.message}
              </span>
            </motion.div>
          )}
        ...
        {sessionStorage.getItem("ferga_quota_exceeded") && (
          <div className="fixed top-0 left-0 w-full z-[1000] bg-yellow-500 text-black px-4 py-2 text-center text-xs font-black flex items-center justify-center gap-2 shadow-lg">
            ...
            <button
              onClick={() => {
                sessionStorage.removeItem("ferga_quota_exceeded");
                window.location.reload();
              }}
...
const CulturalQuote = ({ interfaceLang }: { interfaceLang: InterfaceLang }) => {
  const quotes = [
    {
      sorani: "بەردەوام بە، سەرکەوتن نزیکە",
      kurmanji: "Berdewam be, serkeftin nêzîk e",
    },
    {
      sorani: "زمان ناسنامەی نەتەوەیە",
      kurmanji: "Ziman nasnameya neteweyê ye",
    },
    {
      sorani: "کورد فێرە، چونکە لێهاتووە",
      kurmanji: "Kurd fêr e, çunkî jêhatî ye",
    },
  ];
  ...
  return (
    <motion.div
      initial={{ opacity: 0, y: 10 }}
      animate={{ opacity: 1, y: 0 }}
      className="p-4 bg-white/[0.02] border border-white/5 rounded-[1.25rem] text-center relative overflow-hidden my-8"
    >
      ...
      <div className="flex items-center justify-center gap-2">
        ...
        <span className="text-[10px] font-black uppercase theme-primary tracking-widest font-afarin1">
          {interfaceLang === "Sorani" ? "فێرگەی خانی" : "Fêrga Xanî"}
        </span>
        <div className="w-4 h-[1px] bg-white/10" />
      </div>
...
function NavBtn({ active, icon, label, onClick }: any) {
  return (
    <motion.button
      ...
      whileTap={{ scale: 0.98 }}
      onClick={onClick}
      className={`w-full flex items-center gap-3 px-4 py-2.5 md:py-3 rounded-[1.25rem] font-bold text-xs md:text-sm tracking-wide transition-all ${
        active
    ...
    >
      <span
        className={
          active ? "text-white" : "theme-text opacity-30 transition-colors"
        }
      >
        {icon}
      </span>
...
function MobileNavBtn({ active, icon, label, onClick }: any) {
  return (
    ...
    >
      <div
        className={`transition-all duration-300 ${active ? "scale-110 -translate-y-0.5" : ""}`}
      >
        {icon}
      </div>
      {active && (
      ...
      <span
        className={`text-[8px] font-black uppercase tracking-widest mt-1 font-afarin1 transition-opacity ${active ? "opacity-100" : "opacity-0 h-0 hidden"}`}
      >
        {label}
      </span>
    </motion.button>
...
function HeaderIcon({
  ...
  return (
    <motion.div className="flex items-center gap-2.5 px-3 py-1.5 hover:bg-white/5 rounded-xl transition-colors group cursor-default">
      <div className="text-white/40 transition-transform group-hover:scale-110 group-hover:text-white">
        {React.cloneElement(icon as React.ReactElement, { size: 14 })}
      </div>
      <div className="flex flex-col items-start">
        <span className="font-black text-sm leading-none text-white/90 font-afarin1">
          {val}
        </span>
        {label && (
          <span className="text-[9px] font-bold uppercase theme-primary tracking-wider mt-0.5 opacity-60">
            {label}
          </span>
        )}
      </div>
